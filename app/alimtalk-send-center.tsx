...
import {LinkedAlimtalkBody} from "./alimtalk-record-links";
...
type ReadyStudent={sourceVersion:string;studentId:string;studentName:string;school:string;grade:string;expectedCount:number;completedCount:number;complete:boolean;missingItems:MissingItem[];lessons:Lesson[];recipient:Recipient};
...
export function AlimtalkSendCenter({supabase}:{supabase:SupabaseClient;students:StudentRow[]}){
  ...
  async function sendRow(row:ReadyStudent){const itemPreview=buildPreview(row.studentName,period.start,type,row.lessons);const lengthError=previewLengthError(itemPreview);if(lengthError)return lengthError;const{error}=await supabase.functions.invoke("send-learning-alimtalk",{body:{studentId:row.studentId,sourceVersion:row.sourceVersion,reportType:type,periodStart:period.start,periodEnd:period.end,lessonSummary:itemPreview.lesson,attendanceSummary:itemPreview.attendance,examSummary:itemPreview.exam,homeworkSummary:itemPreview.homework,learningSummary:itemPreview.learningDetails}});if(!error)return null;let text=error.message;const context=(error as{context?:Response}).context;if(context)try{const body=await context.clone().json() as{error?:string};if(body.error)text=body.error}catch{}return text}
  ...
  return <section className={styles.center}>
    ...
    {listMode==="ready"?<><div className={styles.layout}>
      ...
      <main className={styles.workspace}>
        ...
        {!student&&!loading?<Empty text="이 기간에 예정된 수업이 있는 학생이 없습니다."/>:student?<>
          ...
          <section className={styles.preview}><div className={styles.kakaoHead}><span>한살매 수업노트</span><small>알림톡 도착 화면 미리보기</small></div><article><LinkedAlimtalkBody key={student.studentId+period.start+period.end} body={preview.body} supabase={supabase} studentId={student.studentId} studentName={student.studentName} from={period.start} to={period.end}/><button>학습기록 확인</button></article></section>
          {lengthError&&<p role="alert" className={styles.historyError}>{lengthError}</p>}
          ...
          <footer className={styles.actions}><div>{message&&<p className={message.includes("접수")?styles.success:styles.error}>{message}</p>}{sentRecord?.status==="sent"&&<span>이 기간은 이미 발송되었습니다.</span>}</div><button className={styles.primary} onClick={sendCurrent} disabled={sending||Boolean(lengthError)||!student.recipient.available||!student.lessons.length||sentRecord?.status==="sent"}>{sending?"발송 중…":sentRecord?.status==="sent"?"발송 완료":student.complete?"이 학생만 발송":"작성된 기록만 발송"}</button></footer>
        </>:<Empty text="완료 기록을 확인하고 있습니다…"/>}
      </main>
    </div>
    <div className={styles.bulkBar}><div><b>선택 학생 {checkedRows.length}명</b><span>완료 학생은 자동 선택되며, 미완료 학생은 직접 선택할 수 있습니다.</span></div><button disabled={!checkedRows.length||sending} onClick={()=>setConfirming(true)}>{sending?"일괄 발송 중…":`선택 ${checkedRows.length}명 일괄 발송`}</button></div></>:<section className={styles.sentPanel}><header><div><small>{type==="daily"?"DAILY HISTORY":"WEEKLY HISTORY"}</small><h2>{type==="daily"?"일간 발송 완료":"주간 발송 완료"}</h2><p>{formatPeriod(period.start,period.end)} · 행을 누르면 실제 발송 내용을 확인할 수 있습니다.</p></div><span>{visibleSentRows.length}명</span></header><div className={styles.sentList}>{visibleSentRows.map(item=>{const expanded=expandedHistoryId===item.id;return <article key={item.id} className={expanded?styles.historyExpanded:""}><button className={styles.historyRow} aria-expanded={expanded} onClick={()=>setExpandedHistoryId(expanded?"":item.id)}><i><HansalmaeIcon name="chat" size={18}/></i><span><b>{item.studentName}</b><small>{type==="daily"?"일간 학습기록":"주간 학습요약"} · {item.maskedPhone}</small></span><time>{formatSentAt(item.sentAt)}</time><em>{item.sendCount>1?`발송 ${item.sendCount}회`:"발송 완료"}</em><strong aria-hidden="true">{expanded?"−":"+"}</strong></button>{expanded&&<div className={styles.historyDetail}><header><b>실제 발송 내용</b><span>발송 당시 저장된 내용입니다.</span></header><LinkedAlimtalkBody key={item.id} body={buildHistoryBody(item)} supabase={supabase} studentId={item.studentId} studentName={item.studentName} deliveryId={item.id} from={item.periodStart} to={item.templateVariables.periodEnd||item.periodStart}/><footer><span>재발송 시 위 내용을 변경 없이 같은 학부모 연락처로 보냅니다.</span><button disabled={sending} onClick={()=>setResendTarget(item)}>동일 내용 재발송</button></footer></div>}</article>})}{!visibleSentRows.length&&<Empty text={loading?"발송 이력을 확인하고 있습니다…":"이 기간에 발송 완료된 학생이 없습니다."}/>}</div></section>}
    ...
    {resendTarget&&<div className={styles.confirmBackdrop} onMouseDown={event=>{if(event.target===event.currentTarget)setResendTarget(null)}}><section className={styles.confirm} role="alertdialog" aria-modal="true"><i><HansalmaeIcon name="refresh" size={25}/></i><h2>{resendTarget.studentName} 학생 알림톡을 다시 보낼까요?</h2><p>현재 기록을 새로 만들지 않고, {formatSentAt(resendTarget.sentAt)}에 저장된 내용 그대로 {resendTarget.maskedPhone} 번호로 재발송합니다.</p><div><button className={styles.secondary} onClick={()=>setResendTarget(null)}>취소</button><button className={styles.primary} onClick={()=>void resendHistory()}>동일 내용 재발송</button></div></section></div>}
...
function previewLengthError(preview:Preview){const length=Array.from(preview.body).length;return length>1000?`알림톡 전체 내용이 ${length}자로 발송 한도 1,000자를 ${length-1000}자 초과했습니다. 수업·시험·숙제 요약을 줄여 주세요.`:""}
function formatCorrectionTask(row:Lesson){
  ...
  if(row.correctionTaskStatus)lines.push(`  수행 상태: ${{completed:"완료",partial:"부분 완료",incomplete:"미완료"}[row.correctionTaskStatus]}`);
