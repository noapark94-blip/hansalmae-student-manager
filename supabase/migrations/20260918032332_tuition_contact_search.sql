...
AS $function$
  with visible as (
    select
      ...
      case when public.is_staff() then s.phone else null end as student_phone,
      case when public.is_staff() then coalesce((select array_agg(distinct g.phone) filter(where g.phone is not null) from public.student_guardians sg join public.guardians g on g.id=sg.guardian_id where sg.student_id=s.id),array[]::text[]) else array[]::text[] end as guardian_phones,
      s.school,
      s.grade,
      coalesce((
  ...
  select jsonb_build_object(
    ...
    'items', coalesce(jsonb_agg(jsonb_build_object(
      ...
      'guardianPhones', v.guardian_phones,
      'school', v.school,
      'grade', v.grade,
      'subjects', v.subjects,
