-- Assistants use the same staff notification center as teachers, so they must
-- be able to attach the current PWA push endpoint to their own profile too.
...
using (
  ...
  and exists(
    ...
    where p.id=(select auth.uid())
      and p.role::text in ('guardian','teacher','assistant','admin','manager')
  )
)
with check (
  profile_id=(select auth.uid())
...
begin
  ...
  if current_role not in ('guardian','teacher','assistant','admin','manager') then
