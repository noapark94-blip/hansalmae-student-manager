...
export function FamilyExamGrowth({supabase,studentId}:{supabase:SupabaseClient;studentId:string}){
  ...
  ]).then(([regularResult,correctionResult])=>{if(active){setRegular((regularResult.data??[]) as Exam[]);setCorrection((correctionResult.data??[]) as Exam[]);setLoading(false);}});return()=>{active=false};},[studentId,supabase]);
  if(loading)return null;
  return <div className="family-growth-split">
    <ExamGrowthCard title="정규수업 시험 성적 추이" eyebrow="정규수업 성적 성장" empty="아직 정규수업 시험 점수가 없습니다." items={regular}/>
    <ExamGrowthCard title="첨삭수업 시험 성적 추이" eyebrow="첨삭수업 성적 성장" empty="아직 첨삭수업 시험 점수가 없습니다." items={correction}/>
  </div>;
}

function ExamGrowthCard({title,eyebrow,empty,items}:{title:string;eyebrow:string;empty:string;items:Exam[]}){
  ...
  const average=recent.length?Math.round(recent.reduce((sum,x)=>sum+(x.percent??0),0)/recent.length*10)/10:null;
  ...
  return <section className="panel family-growth-card">
    ...
    {!subjects.length?<p className="family-list-empty">{empty}</p>:<>
      <div className="family-growth-summary"><span><small>최근 3회 평균</small><b>{average===null?"–":`${average}점`}</b></span><span><small>직전 시험 대비</small><b className={delta===null?"":delta>0?"up":delta<0?"down":""}>{delta===null?"–":delta>0?`+${delta}점`:delta===0?"변동 없음":`${delta}점`}</b></span><span><small>기록된 시험</small><b>{rows.length}회</b></span></div>
      {rows.length?<div className="family-growth-chart" aria-label={`${selected} ${title}`}>{rows.map((item,index)=><div key={item.id} className="family-growth-bar"><span><i style={{height:`${Math.max(6,Math.min(100,item.percent??0))}%`}}/><em>{formatPercent(item.percent)}</em></span><small>{shortDate(item.lessonDate)}</small><b title={item.examTitle??item.className}>{item.examTitle||item.className}</b>{index===rows.length-1&&<strong>최근</strong>}</div>)}</div>:<p className="family-list-empty">{empty}</p>}
    </>}
  </section>;
}
