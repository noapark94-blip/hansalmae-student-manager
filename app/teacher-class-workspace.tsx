    ...
    <div className="modal-backdrop">
      <form className="modal class-creator" onSubmit={submit}>
        ...
        <div className="modal-body">
          ...
          <fieldset className="schedule-builder"><legend>수업 요일·시간 *</legend>{schedules.map((item,index)=><div className="schedule-row" key={index}><div className="weekday-field"><b>요일</b><div className="weekday-options">{weekdays.map((label,dayIndex)=><button type="button" key={label} className={item.weekdays.includes(dayIndex+1)?"active":""} onClick={()=>updateSchedule(index,{weekdays:item.weekdays.includes(dayIndex+1)?item.weekdays.filter((day)=>day!==dayIndex+1):[...item.weekdays,dayIndex+1]})}>{label}</button>)}</div></div><label>시작<MilitaryTimeInput value={item.startTime} onChange={(value)=>updateSchedule(index,{startTime:value})}/></label><span aria-hidden="true">→</span><label>종료<MilitaryTimeInput value={item.endTime} onChange={(value)=>updateSchedule(index,{endTime:value})}/></label>{schedules.length>1&&<button type="button" className="danger-link" onClick={()=>setSchedules((current)=>current.filter((_,idx)=>idx!==index))}>삭제</button>}</div>)}<small className="schedule-time-hint">시간은 24시간제 4자리로 입력해 주세요. 예: 1800</small><button type="button" className="add-schedule-row" onClick={addSchedule}>＋ 다른 시간 추가</button></fieldset>
        </div>
        {error && <p className="form-error">{error}</p>}
