-- 한살매 중·고등용 키즈노트: 선생님 수업 기록을 학생·학부모 학습리포트로 제공합니다.

create or replace function public.family_learning_reports(
...
begin
  ...
  from (
    select
      jsonb_build_object(
        ...
        'exams', coalesce((
          ...
          where er.lesson_id = l.id and er.student_id = selected_id
        ), '[]'::jsonb)
      ) as report_row,
      l.lesson_date,
      l.starts_at
    ...
    join public.enrollments e on e.class_id = c.id and e.student_id = selected_id
    left join public.profiles tp on tp.id = l.teacher_profile_id
    left join public.attendance a on a.lesson_id = l.id and a.student_id = selected_id
