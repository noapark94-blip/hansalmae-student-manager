...
function Month({supabase,classId,anchor,onDate,onClose}:{supabase:SupabaseClient;classId:string;anchor:string;onDate:(v:string)=>void;onClose:()=>void}){
  ...
  return <div className="modal-backdrop nested"><section className="student-modal class-month-modal"><header><div><p className="eyebrow">전체 출결 현황</p><h2>출석 캘린더</h2></div><button onClick={onClose}>×</button></header><nav className="month-picker" aria-label="출석 달력 월 이동"><button type="button" aria-label="이전 달" onClick={()=>setMonth(shiftMonth(month,-1))}>‹</button><b>{formatMonth(month)}</b><button type="button" aria-label="다음 달" onClick={()=>setMonth(shiftMonth(month,1))}>›</button><button type="button" onClick={()=>setMonth(koreaMonth())}>이번 달</button></nav><div className="month-calendar"><div className="month-weekdays">{weekdays.map(day=><b key={day}>{day}</b>)}</div><div className="month-days" style={{"--first-offset":offset} as CSSProperties}>{days.map(day=><button key={day.date} className={day.scheduled?"scheduled":""} onClick={()=>onDate(day.date)}><b>{+day.date.slice(8)}</b><span>{day.students.map(student=><em className={student.status==="excused"?"absent":student.status} key={student.id}>{student.name}</em>)}</span></button>)}</div></div></section></div>;
}
