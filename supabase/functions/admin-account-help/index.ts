...
Deno.serve(async request=>{
  ...
  const{data:profiles,error:profileError}=await authClient.rpc("admin_account_help_target",{p_profile_id:input.profileId});const profile=profiles?.[0];if(profileError){console.error("[admin-account-help] profile lookup failed",{profileId:input.profileId,error:profileError.message,code:profileError.code});return json({error:"선택한 계정 정보를 불러오지 못했습니다. 잠시 후 다시 시도해 주세요."},500)}if(!profile)return json({error:"선택한 계정이 비활성화됐거나 삭제되었습니다. 다른 계정을 선택해 주세요."},404);
  ...
  const{data:prepared,error:prepareError}=await authClient.rpc("admin_prepare_account_help",{p_request_id:help.id,p_profile_id:profile.id,p_phone:help.reachable_phone});if(prepareError||prepared!==true){console.error("[admin-account-help] prepare failed",{requestId:help.id,profileId:profile.id,error:prepareError?.message??null});return json({error:"계정 연락처를 변경하지 못했습니다. 잠시 후 다시 시도해 주세요."},500)}
  ...
  const{data:finished,error:finishError}=await authClient.rpc("admin_finish_account_help",{p_request_id:help.id,p_profile_id:profile.id});if(finishError||finished!==true){console.error("[admin-account-help] finish failed",{requestId:help.id,profileId:profile.id,error:finishError?.message??null});return json({error:"문자는 발송됐지만 요청 완료 처리를 하지 못했습니다. 화면을 새로고침해 주세요."},500)}
