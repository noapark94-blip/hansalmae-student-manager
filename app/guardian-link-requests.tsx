...
export function GuardianLinkRequests({supabase}:{supabase:SupabaseClient}){
  ...
  const resolve=async(row:RequestRow,action:"approve"|"reject")=>{const studentId=selected[row.id]||"";if(action==="approve"&&!studentId){setMessage("연결할 학생을 선택해 주세요.");return}if(!await appConfirm({eyebrow:action==="approve"?"자녀 연결 승인":"연결 요청 거절",title:action==="approve"?`${row.guardianName} 학부모님을 선택한 학생과 연결할까요?`:`${row.guardianName} 학부모님의 요청을 거절할까요?`,notice:action==="approve"?"승인하면 학부모 계정에서 해당 학생의 정보를 볼 수 있습니다.":"거절한 요청은 목록에서 사라집니다.",confirmLabel:action==="approve"?"연결 승인":"요청 거절",tone:action==="reject"?"danger":"default"}))return;setProcessing(row.id);setMessage("");const{error}=await supabase.rpc("admin_resolve_guardian_link_request",{p_request_id:row.id,p_action:action,p_student_id:studentId||null});setMessage(error?error.message:action==="approve"?"학부모 계정을 승인하고 자녀를 연결했습니다.":"연결 요청을 거절했습니다.");if(!error)await load();setProcessing("")};
