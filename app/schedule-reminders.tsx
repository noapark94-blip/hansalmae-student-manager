...
export function ScheduleReminderList(){const state=useScheduleReminders();if(!state)return null;return <div className={styles.list}><div className={styles.toolbar}><span>최근 30일 · 미확인 {state.items.filter(r=>!r.read).length}건</span><button onClick={()=>void state.refresh()}>새로고침</button></div>{state.error&&<p role="alert">{state.error}</p>}{state.loading?<p>일정 알림을 확인하고 있어요.</p>:!state.items.length?<div className={styles.empty}><b>등록된 일정 알림이 없습니다</b><p>당일 알림을 켠 담당 일정이 여기에 모입니다.</p></div>:state.items.map(r=><div className={styles.entry} key={key(r)}><button className={`${styles.item} ${r.read?styles.read:""}`} onClick={()=>state.open(r)}><span>{r.kind}<em>{r.read?"확인 완료":"미확인"}</em></span><b>{r.title}</b><small>{r.date} · {r.time??"시간 미지정"}{r.place?` · ${r.place}`:""}</small>{r.person&&<p>{r.person}</p>}</button>{r.read&&<button className={styles.deleteReminder} disabled={state.busy} aria-label={`${r.title} 알림 삭제`} onClick={()=>void state.dismiss(r)}>알림 삭제</button>}</div>)}</div>;}
