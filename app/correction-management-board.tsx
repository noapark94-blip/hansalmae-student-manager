...
function buildOccurrences(data:Board|null):Occurrence[]{if(!data)return[];return (data.assignments??[]).flatMap(a=>{const date=addDays(data.weekStart,a.weekday-1);return isAssignmentValid(a,date)?[{key:`fixed-${a.id}-${date}`,assignment:a,date,startTime:a.startTime,endTime:a.endTime,state:"fixed" as const}]:[]})}
