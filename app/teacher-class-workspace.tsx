      ...
      <div className="class-workspace-grid class-log-grid">
        <section className="class-roster-shortcut">
          ...
          <div className="class-roster-actions">
            {date<=today()?<button type="button" className="secondary-button lesson-roster-button" onClick={() => setLessonRosterOpen(true)}>이 날짜 누락 학생</button>:null}
            <button type="button" className="secondary-button" onClick={() => setRosterOpen(true)}>학생 추가·제외</button>
          </div>
        </section>
      </div>
...
function LessonRosterOverrideEditor({supabase,classRoom,date,onClose,onChanged}:{supabase:SupabaseClient;classRoom:ClassRoom;date:string;onClose:()=>void;onChanged:()=>Promise<void>}){
  ...
  return <div className="nested-modal-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget)onClose()}}><section className="modal lesson-roster-editor" role="dialog" aria-modal="true" aria-labelledby="lesson-roster-title"><header><div><p className="eyebrow">날짜별 명단 보정</p><h2 id="lesson-roster-title">{date} 누락 학생</h2><p>{classRoom.name}의 이 날짜 기록에만 학생을 추가합니다. 실제 수강 시작일은 바뀌지 않습니다.</p></div><button type="button" className="modal-close" aria-label="닫기" onClick={onClose}>×</button></header><div className="modal-body"><div className="lesson-roster-notice"><b>과거 기록 보정용</b><span>기본 명단 학생은 그대로 유지되며, 직접 추가한 학생만 다시 제외할 수 있습니다.</span></div><input className="roster-search" value={query} onChange={event=>setQuery(event.target.value)} placeholder="학생 이름 또는 학교 검색"/>{loading?<p className="empty-state">학생을 불러오는 중이에요…</p>:<div className="lesson-roster-option-list">{visible.map(student=>{const included=student.standardIncluded||student.directlyAdded;return <article key={student.id} className={student.directlyAdded?"direct-added":included?"standard":""}><i>{student.name.slice(0,1)}</i><span><b>{student.name}</b><small>{[student.school,student.grade].filter(Boolean).join(" · ")||"학교·학년 미입력"}</small></span>{student.standardIncluded?<em>기본 명단</em>:<button type="button" disabled={saving===student.id} className={student.directlyAdded?"remove":"add"} onClick={()=>void toggle(student)}>{saving===student.id?"처리 중…":student.directlyAdded?"직접 추가 취소":"이 날짜에 추가"}</button>}</article>})}{!visible.length?<p className="empty-state">검색 결과가 없습니다.</p>:null}</div>}{error?<p className="form-error">{error}</p>:null}</div><footer><button type="button" className="primary" onClick={onClose}>완료</button></footer></section></div>
}
    ...
    <div className="modal-backdrop">
      <form className="modal class-creator" onSubmit={submit}>
        ...
        <div className="modal-body">
          ...
          <fieldset className="teacher-picker"><legend>담당 선생님{profile.role !== "admin" ? " *" : ""} <small>{profile.role === "admin" ? "나중에 배정 가능" : "공동 담당 가능"}</small></legend><div>{teachers.map((item)=><label key={item.id} className={teacherIds.includes(item.id)?"active":""}><input type="checkbox" checked={teacherIds.includes(item.id)} onChange={()=>setTeacherIds((current)=>current.includes(item.id)?current.filter((id)=>id!==item.id):[...current,item.id])}/><i>{item.name.slice(0,1)}</i><span>{item.name}</span></label>)}</div></fieldset>
