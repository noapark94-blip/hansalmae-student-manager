...
type Dashboard = { counts: Record<string, number>; tableCounts?: Record<string, number>; exportedTableCount?: number; databaseOnlyTableCount?: number; recentLogs: { id: string; action: string; section: string; recordCount: number; createdAt: string; requestedBy: string }[] };
...
const sections = [
  { id: "students", icon: "학생", title: "학생·보호자", desc: "학생 기본 정보, 보호자 연결, 학교 및 재원 이력", tables: 11 },
  { id: "classes", icon: "수업", title: "클래스·계정", desc: "클래스, 시간표, 수강 배정 및 계정 설정", tables: 14 },
  { id: "learning", icon: "기록", title: "학습·상담 기록", desc: "출결, 보강, 과제, 시험, 특강 및 상담", tables: 22 },
  { id: "operations", icon: "운영", title: "운영 시간표", desc: "첨삭 배정·보고서, 차량 운행 및 예외 일정", tables: 14 },
  { id: "billing", icon: "수납", title: "수납", desc: "청구, 납부, 할인·추가금 및 수강료 정책", tables: 6 },
  { id: "communications", icon: "소식", title: "공지·문자", desc: "공지와 읽음 상태, 문자 발송 및 가족 알림", tables: 6 },
  { id: "vocabulary", icon: "단어", title: "단어 시험", desc: "단어 세트, 전체 단어 및 최근 시험 출제 이력", tables: 3 },
] as const;
...
export function BackupBoard({ supabase }: { supabase: SupabaseClient }) {
  ...
  const validate = async (file: File) => {
    ...
    try {
      ...
      if (parsed?.metadata?.app !== "hansalmae-student-manager" || ![1, 2, 3].includes(parsed.metadata.version) || !parsed.data || typeof parsed.data !== "object") throw new Error();
  ...
  return <div className="backup-page">
    <section className="backup-hero"><div><p className="eyebrow">관리자 전용 · 데이터 보호</p><h1>데이터 백업</h1><p>현재 운영 중인 데이터를 영역별로 내보내고, 복구용 SQL 백업 방법까지 한곳에서 확인합니다.</p><div className="backup-summary"><span><b>{dashboard ? totalRecords.toLocaleString("ko-KR") : "–"}</b>전체 레코드</span><span><b>{dashboard?.exportedTableCount ?? totalTables}</b>내보내기 테이블</span><span><b>{dashboard?.databaseOnlyTableCount ?? "–"}</b>SQL 전용 테이블</span></div></div><button className="backup-main-action" disabled={!!busy} onClick={() => void exportData("all", "json")}><span>{busy === "all-json" ? "생성 중…" : "전체 운영 데이터"}</span><small>JSON으로 내보내기</small></button></section>
    ...
    <div className="backup-tools">
      ...
      <section className="backup-tool backup-sql"><div className="backup-tool-icon">SQL</div><div><h2>복구용 SQL 백업</h2><p>DB 구조·실제 데이터·역할과 JSON에서 제외한 보안 테이블까지 저장합니다. Storage의 실제 파일과 외부 인증 설정은 별도 보관이 필요합니다.</p></div><code>.\scripts\backup-supabase.ps1</code><small>Windows PowerShell · 프로젝트 폴더에서 실행</small></section>
    </div>
    ...
    <p className="backup-footnote">JSON은 {dashboard?.exportedTableCount ?? totalTables}개 운영 테이블을 보관합니다. 복구 코드·푸시 기기 키·백업 이력 등 {dashboard?.databaseOnlyTableCount ?? 4}개 보안·내부 테이블은 노출을 막기 위해 SQL 백업에만 포함합니다.</p>
  </div>;
}
