...
export function StudentAcademicRecords({supabase,studentId,studentGrade}:{supabase:SupabaseClient;studentId:string;studentGrade:string}) {
  ...
  return <section className="academic-records">
    ...
    {!loading&&visible.length?<><div className="academic-summary"><article><span>최근 시험</span><b>{latest.exam_name}</b><small>{latest.exam_date}</small></article><article><span>최근 원점수</span><b>{latest.score??"-"}<small>{latest.score!==null?"점":""}</small></b><small>{latest.subject}</small></article><article><span>{latest.achievement_level?"최근 성취도":"최근 등급"}</span><b>{latest.achievement_level??latest.grade??"-"}<small>{latest.grade!==null?"등급":""}</small></b><small>{visible.length}건 기록</small></article></div><section className="academic-trend"><header><b>최근 성적 변화</b><span>{type==="mock"?"백분위 우선":"원점수 기준"}</span></header><div>{trend.map(item=>{const value=type==="mock"?(item.percentile??item.score):(item.score??null);return <article key={item.id}><span>{item.exam_name}<small>{item.subject}</small></span><i><em style={{width:`${value??0}%`}}/></i><b>{value===null?"-":value}</b></article>})}</div></section><div className="academic-list">{visible.map(item=><article key={item.id}><div><span>{item.academic_year}년{item.semester?` · ${item.semester}학기`:""} · {item.exam_date}</span><b>{item.exam_name} · {item.subject}</b><p>{item.score!==null?`원점수 ${item.score}`:""}{item.standard_score!==null?` · 표준점수 ${item.standard_score}`:""}{item.percentile!==null?` · 백분위 ${item.percentile}`:""}{item.achievement_level?` · 성취도 ${item.achievement_level}`:""}{item.grade!==null?` · ${item.grade}등급`:""}{item.rank!==null?` · ${item.rank}/${item.cohort_size??"-"}등`:""}{item.school_average!==null?` · 학교 평균 ${item.school_average}`:""}</p>{item.note?<small>{item.note}</small>:null}</div><footer><button type="button" onClick={()=>openEdit(item)}>수정</button><button type="button" className="danger" onClick={()=>setDeleteId(item.id)}>삭제</button></footer>{deleteId===item.id?<div className="academic-delete"><span>이 성적 기록을 삭제할까요?</span><div><button type="button" onClick={()=>setDeleteId(null)}>취소</button><button type="button" className="danger-fill" disabled={saving} onClick={()=>void remove(item.id)}>삭제</button></div></div>:null}</article>)}</div></>:loading?<p className="academic-empty">성적 기록을 불러오는 중이에요…</p>:<p className="academic-empty">아직 등록된 {type==="school"?"내신":"모의고사·수능"} 성적이 없습니다.</p>}
  </section>;
}
