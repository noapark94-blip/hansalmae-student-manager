...
function ClassRosterEditor({ supabase, classRoom, onClose, onSaved }: { supabase: SupabaseClient; classRoom: ClassRoom; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  return <div className="nested-modal-backdrop"><div className="modal roster-editor"><header><div><p className="eyebrow">수강학생 관리</p><h2>{classRoom.name}</h2><p>학생을 추가한 뒤, 실제로 참석하는 요일·시간만 따로 설정할 수 있습니다.</p></div><button className="modal-close" aria-label="닫기" onClick={onClose}>×</button></header><div className="modal-body"><input className="roster-search" value={query} onChange={(e)=>setQuery(e.target.value)} placeholder="학생 이름 또는 학교 검색"/>{loading?<p className="empty-state">학생을 불러오는 중이에요…</p>:<div className="roster-student-list">{sorted.map((item)=>{const enrolled=enrolledIds.has(item.id);return <article key={item.id} className={enrolled?"enrolled":""}><span><b>{item.name}</b><small>{[item.school,item.grade].filter(Boolean).join(" · ")||"학교·학년 미입력"}</small></span><div>{enrolled&&classRoom.schedules.length>1?<button type="button" className="schedule-button" onClick={()=>setScheduleStudent(item)}>요일 설정</button>:null}<button type="button" className={enrolled?"remove-button":"add-button"} disabled={saving===item.id} onClick={()=>void toggle(item.id,enrolled)}>{saving===item.id?"처리 중…":enrolled?"제외":"추가"}</button></div></article>})}{!sorted.length?<p className="empty-state">검색 결과가 없습니다.</p>:null}</div>}{error&&<p className="form-error">{error}</p>}</div><footer><button className="primary" onClick={onClose}>완료</button></footer>{scheduleStudent?<ClassStudentScheduleEditor supabase={supabase} classRoom={classRoom} student={scheduleStudent} onClose={()=>setScheduleStudent(null)} onSaved={async()=>{setScheduleStudent(null);await onSaved()}}/>:null}</div></div>
}

function ClassStudentScheduleEditor({supabase,classRoom,student,onClose,onSaved}:{supabase:SupabaseClient;classRoom:ClassRoom;student:RosterStudent;onClose:()=>void;onSaved:()=>Promise<void>}){
