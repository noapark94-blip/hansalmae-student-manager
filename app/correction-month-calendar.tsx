...
export function CorrectionMonthCalendar({supabase,anchor,onSelect,onClose}:{supabase:SupabaseClient;anchor:string;onSelect:(date:string)=>void;onClose:()=>void}){
  ...
  const load=useCallback(async()=>{
    ...
    const assignmentMap=new Map<string,Assignment>();for(const b of parsed)for(const a of b.assignments??[])assignmentMap.set(a.id,{...a,closedDates:[...new Set([...(assignmentMap.get(a.id)?.closedDates??[]),...(a.closedDates??[])])]});setAssignments([...assignmentMap.values()]);
...
function buildOccurrences(assignments:Assignment[],exceptions:Exception[],date:string):Occurrence[]{const weekday=isoWeekday(date);const rows:Occurrence[]=[];for(const a of assignments){if(a.weekday===weekday&&isAssignmentValid(a,date)){const x=exceptions.find(e=>e.assignmentId===a.id&&e.originalDate===date&&(e.kind==="move"||e.kind==="cancel"));if(!x)rows.push({assignment:a,date,startTime:a.startTime,endTime:a.endTime})}}for(const x of exceptions){if((x.kind==="move"||x.kind==="extra")&&x.targetDate===date&&x.targetStartTime&&x.targetEndTime){const a=assignments.find(v=>v.id===x.assignmentId);if(a)rows.push({assignment:a,date,startTime:x.targetStartTime,endTime:x.targetEndTime})}}return rows.filter(r=>!r.assignment.closedDates?.includes(date)).sort((a,b)=>a.startTime.localeCompare(b.startTime)||a.assignment.studentName.localeCompare(b.assignment.studentName,"ko"))}
