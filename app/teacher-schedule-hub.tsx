...
type HubData = { teachers: Named[]; students: Named[]; classes: Named[]; classSchedules: ClassSchedule[]; corrections: Correction[]; correctionExceptions: CorrectionException[]; correctionCapacities:CorrectionCapacity[]; vehicles: VehicleRun[];vehicleExceptions:VehicleException[];todayVehicles:VehicleRun[] };
type Editor = { kind: "class"; row?: ClassSchedule } | { kind: "correction"; row?: Correction } | { kind: "exception"; row: Correction } | {kind:"capacity"}| { kind: "vehicle"; row?: VehicleRun }|{kind:"vehicleException";row:VehicleRun} | null;

const emptyData: HubData = { teachers: [], students: [], classes: [], classSchedules: [], corrections: [], correctionExceptions: [], correctionCapacities:[], vehicles: [],vehicleExceptions:[],todayVehicles:[] };
...
function CorrectionBoard({ rows, exceptions, capacities, profile, onAdd,onCapacity, onEdit, onException }: { rows: Correction[]; exceptions: CorrectionException[]; capacities:CorrectionCapacity[]; profile: Profile; onAdd: () => void;onCapacity:()=>void; onEdit: (row: Correction) => void; onException: (row: Correction) => void }) {
  ...
  return <section className="panel hub-panel"><HubToolbar title="고정 첨삭 시간표" description="월–금 17:30부터 90분 단위이며, 담당쌤만 자신의 배정과 정원을 변경할 수 있습니다."><><button className="secondary-button" onClick={onCapacity}>내 정원 설정</button><button className="primary hub-add" onClick={onAdd}>＋ 내 학생 배정</button></></HubToolbar>{upcoming.length > 0 && <div className="exception-strip"><b>예정된 주간 변경</b>{upcoming.map((item) => { const assignment = rows.find((row) => row.id === item.assignmentId); return <span key={item.id}>{item.weekStart} · {assignment?.studentName ?? "학생"} → {weekdays[item.weekday - 1]} {correctionSlots[item.slotIndex]}</span>; })}</div>}<div className="correction-grid"><div className="correction-corner">시간</div>{weekdays.slice(0,5).map((day) => <b key={day}>{day}</b>)}{correctionSlots.flatMap((slot, slotIndex) => [<strong key={`${slot}-label`}>{slot}</strong>, ...weekdays.slice(0,5).map((day, dayIndex) => <div key={`${day}-${slot}`} className="correction-cell"><em className="slot-capacity">내 배정 {rows.filter((row)=>row.teacherId===profile.id&&row.weekday===dayIndex+1&&row.slotIndex===slotIndex).length}/{capacityFor(capacities,profile.id,dayIndex+1,slotIndex)}</em>{rows.filter((row) => row.weekday === dayIndex + 1 && row.slotIndex === slotIndex).map((row) => <span key={row.id} className={row.teacherId === profile.id ? "editable" : ""}><b>{row.studentName}</b><small>{row.teacherName}쌤</small>{row.teacherId === profile.id && <i><button onClick={() => onEdit(row)}>고정 변경</button><button onClick={() => onException(row)}>이번 주만</button></i>}</span>)}</div>)])}</div>{rows.length === 0 && <Empty text="아직 배정된 고정 첨삭 시간이 없습니다." />}</section>;
}

function VehicleBoard({ rows,todayRows,exceptions, onAdd, onEdit,onException }: { rows: VehicleRun[];todayRows:VehicleRun[];exceptions:VehicleException[]; onAdd: () => void; onEdit: (row: VehicleRun) => void;onException:(row:VehicleRun)=>void }) {
  const upcoming=exceptions.filter((item)=>item.serviceDate>=getToday()).slice(0,6);
  return <section className="panel hub-panel"><HubToolbar title="차량 운행 시간표" description="고정 노선과 날짜별 변경을 분리하고, 휴원·퇴원 학생은 오늘 운행에서 자동 제외합니다."><button className="primary hub-add" onClick={onAdd}>＋ 정류장 등록</button></HubToolbar>{upcoming.length>0&&<div className="exception-strip"><b>예정된 차량 변경</b>{upcoming.map((item)=>{const run=rows.find((row)=>row.id===item.runId);return <span key={item.id}>{item.serviceDate} · {run?.routeName??"노선"} {run?.pickupLocation??""} → {item.kind==="cancelled"?"운행 취소":`${item.pickupTime?.slice(0,5)} ${item.pickupLocation}`}</span>})}</div>}<div className="today-vehicle-board"><header><b>오늘 운행</b><span>탑승 학생 {new Set(todayRows.flatMap((row)=>row.students.map((student)=>student.id))).size}명</span></header>{todayRows.length?todayRows.map((row)=><article key={row.id}><strong>{row.stopOrder}</strong><time>{row.pickupTime.slice(0,5)}</time><span><b>{row.routeName} · {row.direction==="pickup"?"승차":"하차"}</b><small>{row.pickupLocation} · 차량실장님 {row.managerName}{row.changed?" · 오늘만 변경":""}</small><em>{row.students.map((student)=>student.name).join(", ")||"탑승학생 없음"}</em></span></article>):<p>오늘 예정된 차량 운행이 없습니다.</p>}</div><div className="vehicle-week">{weekdays.map((day, index) => <div key={day}><b>{day}</b>{rows.filter((row) => row.weekday === index + 1).map((row) => <article className="vehicle-card" key={row.id}><button className="vehicle-card-main" onClick={() => onEdit(row)}><time>{row.stopOrder}. {row.pickupTime.slice(0,5)}</time><span><strong>{row.routeName} · {row.direction==="pickup"?"승차":"하차"}</strong><small>{row.pickupLocation} · {row.managerName}</small><em>{row.students.map((student) => student.name).join(", ") || "탑승학생 미배정"}</em></span></button><button className="vehicle-exception-button" onClick={()=>onException(row)}>날짜별 변경</button></article>)}</div>)}</div>{rows.length === 0 && <Empty text="아직 등록된 차량 운행 일정이 없습니다." />}</section>;
}
...
function CapacityEditor({supabase,profile,capacities,onClose,onSaved}:EditorProps&{profile:Profile;capacities:CorrectionCapacity[]}){
  const[weekday,setWeekday]=useState(1);const[slotIndex,setSlotIndex]=useState(0);const[capacity,setCapacity]=useState(capacityFor(capacities,profile.id,1,0));const[saving,setSaving]=useState(false);const[error,setError]=useState("");
  useEffect(()=>setCapacity(capacityFor(capacities,profile.id,weekday,slotIndex)),[capacities,profile.id,slotIndex,weekday]);
  const submit=async(event:FormEvent)=>{event.preventDefault();setSaving(true);setError("");const{error:saveError}=await supabase.from("correction_slot_capacities").upsert({teacher_profile_id:profile.id,weekday,slot_index:slotIndex,capacity,updated_at:new Date().toISOString()},{onConflict:"teacher_profile_id,weekday,slot_index"});if(saveError){setError(saveErrorMessage(saveError.message,"첨삭 정원을 저장하지 못했습니다."));setSaving(false);}else await onSaved();};
  return <EditorModal title="내 첨삭 정원 설정" description="설정하지 않은 시간은 기본 8명이며, 정원이 찬 시간에는 학생을 더 배정할 수 없습니다." onClose={onClose}><form onSubmit={submit}><div className="form-pair"><DaySelect value={weekday} onChange={setWeekday} count={5}/><SlotSelect value={slotIndex} onChange={setSlotIndex}/><FormField label="최대 학생 수"><input type="number" min={1} max={30} value={capacity} onChange={(event)=>setCapacity(Number(event.target.value))} required/></FormField></div>{error&&<p className="form-error">{error}</p>}<EditorFooter saving={saving}/></form></EditorModal>;
}
...
function capacityFor(rows:CorrectionCapacity[],teacherId:string,weekday:number,slotIndex:number){return rows.find((item)=>item.teacherId===teacherId&&item.weekday===weekday&&item.slotIndex===slotIndex)?.capacity??8}
function saveErrorMessage(message: string, fallback: string) { return message.includes("정원") || message.includes("요일") || message.includes("오늘 이후") || message.includes("겹치는") || message.includes("시간 충돌") || message.includes("같은 시간") || message.includes("같은 시각") || message.includes("같은 첨삭") || message.includes("이미 배정") || message.includes("이미 있") || message.includes("늦어야") ? message : fallback; }
