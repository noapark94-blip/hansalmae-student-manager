    ...
    <div className={styles.layout}>
      ...
      <main className={styles.workspace}>
        ...
        {!student&&!loading?<Empty text="이 기간에 예정된 수업이 있는 학생이 없습니다."/>:student?<>
          <div className={styles.readiness}><div><i className={student.complete?styles.ok:styles.warning}>!</i><span><b>{student.complete?"예정 기록 완료":"일부 기록 미작성"}</b><small>{student.completedCount}/{student.expectedCount}건 작성됨</small></span></div><div><i className={student.recipient.available?styles.ok:""}>✓</i><span><b>수신 학부모</b><small>{student.recipient.available?`${student.recipient.guardianName} · ${student.recipient.maskedPhone}`:"등록된 학부모 연락처 없음"}</small></span></div><div><i className={student.lessons.length&&preview.learning.length<=180?styles.ok:""}>✓</i><span><b>내용 자동 생성</b><small>{student.lessons.length?`학습요약 ${preview.learning.length}/180자`:"작성된 기록 없음"}</small></span></div></div>
