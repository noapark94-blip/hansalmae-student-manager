  ...
  return <>
    <nav aria-label="주요 메뉴" className="folder-navigation">
      ...
      {folders.map((folder) => <section className="nav-folder" key={folder.id}>
        ...
        {!collapsed[folder.id] && <div className="nav-folder-items">{folder.itemIds.map((id) => { const item = itemMap.get(id); return item ? <button type="button" key={id} className={activeView === id ? "active" : ""} onClick={() => onSelect(id)}><span className="nav-icon"><HansalmaeIcon name={viewIcon[id]} /></span>{item.label}</button> : null; })}</div>}
      </section>)}
    </nav>
