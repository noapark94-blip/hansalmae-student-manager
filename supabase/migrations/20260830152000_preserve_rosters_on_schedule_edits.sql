-- 클래스 시간표를 수정해도 개인 시간표를 쓰는 기존 수강생의 명단 연결을 보존합니다.
...
begin
  ...
  -- 기존에 이 클래스의 개인 시간표를 쓰던 학생을 삭제 전에 기억합니다.
  select coalesce(array_agg(distinct ssa.student_id),'{}'::uuid[])
  into personal_schedule_student_ids
  from public.student_schedule_assignments ssa
  join public.class_schedules cs on cs.id=ssa.class_schedule_id
  join public.enrollments e on e.student_id=ssa.student_id
  ...
  select selected.student_id,cs.id,auth.uid()
  from unnest(personal_schedule_student_ids) selected(student_id)
  cross join public.class_schedules cs
  where cs.class_id=p_class_id
