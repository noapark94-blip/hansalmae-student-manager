...
import {filterWork,workPeriod,type RecordWorkItem} from './record-work-model';
...
type State={items:RecordWorkItem[];loading:boolean;error:string;refresh:()=>Promise<void>;open:(item:RecordWorkItem)=>void;profile:Profile;supabase:SupabaseClient};
...
function Active({supabase,profile,children}:{supabase:SupabaseClient;profile:Profile;children:ReactNode}){
 ...
 const refresh=useCallback(async()=>{
  ...
  try{const {today,from}=workPeriod();const result=await supabase.rpc('staff_record_worklist',{p_from:from,p_to:today});if(!alive.current)return;if(result.error)throw result.error;if(!Array.isArray(result.data))throw new Error('기록 목록 응답을 확인하지 못했습니다.');
   setItems(result.data);setError('');
   if(result.data.some((x:RecordWorkItem)=>x.due&&x.owners.some(p=>p.id===profile.id))){const claim=await supabase.rpc('staff_claim_record_reminder');if(alive.current&&!claim.error&&claim.data)setToast(claim.data);}
   else setToast(null);
  }catch(e){if(alive.current){setError((e as {message?:string}).message??'기록 목록을 불러오지 못했습니다.');setItems([]);}}
 ...
 return <Context.Provider value={{items,loading,error,refresh,open:setSelected,profile,supabase}}>{children}
...
export function RecordWorkPanel({expanded=false,allDates=false}:{expanded?:boolean;allDates?:boolean}){
 const state=useContext(Context);const [show,setShow]=useState(expanded),[date,setDate]=useState(allDates?'':workPeriod().today),[kind,setKind]=useState('all'),[owner,setOwner]=useState('all'),[busy,setBusy]=useState(false),[message,setMessage]=useState('');
 if(!state)return null;
 const {items,loading,error,refresh,open,profile,supabase}=state,admin=profile.role==='admin';
 ...
 {show&&<><p className={styles.hint}>수업이 끝난 뒤 출결과 완료 처리가 필요한 기록입니다. 시험·숙제 공란은 누락으로 보지 않아요.</p><div className={styles.filters}><label>기록 날짜<select value={date} onChange={e=>setDate(e.target.value)}><option value="">최근 7일 전체</option>{Array.from({length:7},(_,i)=>{const d=new Date(workPeriod().today+'T12:00:00Z');d.setUTCDate(d.getUTCDate()-i);const v=d.toISOString().slice(0,10);return <option key={v} value={v}>{i===0?'오늘 · ':''}{v.slice(5).replace('-','월 ')}일</option>})}</select></label>{admin&&<label>담당자<select value={owner} onChange={e=>setOwner(e.target.value)}><option value="all">전체 담당자</option><option value="mine">내 기록</option>{owners.filter(p=>p.id!==profile.id).map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></label>}<div className={styles.tabs} role="group" aria-label="수업 종류">{[['all','전체'],['regular','정규'],['correction','첨삭'],['makeup','보강'],['additional','추가']].map(([v,label])=><button key={v} aria-pressed={kind===v} onClick={()=>setKind(v)}>{label}</button>)}</div></div>
