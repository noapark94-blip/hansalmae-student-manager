    ...
    <section className="family-exam-trend-modal" role="dialog" aria-modal="true" aria-labelledby="family-exam-trend-title">
      ...
      <div className="family-exam-trend-scroll">
        {loading?<p className="family-exam-trend-state">성적 기록을 불러오는 중이에요…</p>:error?<p className="family-exam-trend-state error">{error}</p>:!subjects.length?<p className="family-exam-trend-state">아직 그래프로 볼 시험 점수가 없어요.</p>:<>
          ...
          {trendRows.length?<section className="family-exam-line-card"><div className="family-exam-chart-caption"><span>{range==='recent'?'개별 시험 점수':range==='all'?'월별 평균':'주별 평균'}</span><em><i />평균 {formatTrendScore(average)}</em></div><div className="family-exam-line-y"><span>100</span><span>{Math.round((100+domainMin)/2)}</span><span>{domainMin}</span></div><div className="family-exam-line-wrap"><svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`${selectedSubject} ${selectedCategory} 점수 추이`} preserveAspectRatio="none"><defs><linearGradient id="familyExamArea" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#a12d66" stopOpacity=".25"/><stop offset="72%" stopColor="#c35b8d" stopOpacity=".07"/><stop offset="100%" stopColor="#fff" stopOpacity="0"/></linearGradient></defs><line className="average" x1={padX} x2={width-padX} y1={averageY} y2={averageY}/><path className="area" d={`${path} L${points.at(-1)?.x},${height-padY} L${points[0]?.x},${height-padY} Z`}/><path className="line-shadow" d={path}/><path className="line" d={path}/>{points.map((point)=><g key={point.item.id} className={selectedPoint?.id===point.item.id?'selected':''}><circle className="dot" cx={point.x} cy={point.y} r={selectedPoint?.id===point.item.id?5.5:4}/>{selectedPoint?.id===point.item.id&&<text x={point.x} y={Math.max(15,point.y-15)} textAnchor="middle">{formatTrendScore(point.item.percent)}</text>}</g>)}</svg><div className="family-exam-line-points">{points.map((point)=><button type="button" key={point.item.id} style={{left:`${(point.x/width)*100}%`,top:`${(point.y/height)*100}%`}} aria-label={`${point.item.label} ${formatTrendScore(point.item.percent)}`} onClick={()=>selectPoint(point.item.id)} />)}</div><div className="family-exam-line-dates">{trendRows.map((item,index)=>(index%labelStep===0||index===trendRows.length-1)?<span key={item.id} style={{left:`${trendRows.length===1?50:(index/(trendRows.length-1))*100}%`}}>{item.label}</span>:null)}</div></div></section>:<p className="family-exam-trend-state compact">선택한 기간의 점수 기록이 없어요.</p>}
