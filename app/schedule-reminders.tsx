...
function ActiveProvider({supabase,profile,children}:{supabase:SupabaseClient;profile:Profile;children:ReactNode}){
 ...
 {typeof document!=="undefined"&&first&&!selected&&createPortal(<aside className={styles.toast} role="status">
  <span className={styles.toastIcon} aria-hidden="true"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="5" width="18" height="16" rx="3"/><path d="M16 3v4M8 3v4M3 11h18m-13 5 2 2 4-4"/></svg></span>
  <div className={styles.toastContent}><small>오늘의 일정 <span>{today.length}건</span></small><h3>{today.length>1?"확인할 일정이 있어요":`${first.kind} 일정 안내`}</h3></div>
  <button type="button" className={styles.close} aria-label="알림 카드 닫기" onClick={()=>setPopup([])}><svg aria-hidden="true" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round"><path d="m6 6 12 12M18 6 6 18"/></svg></button>
  <div className={styles.toastInfo}><span className={styles.toastTime}>{first.time??"시간 미지정"}</span><div><p>{first.title}</p>{first.person&&<p className={styles.person}>{first.person}</p>}</div></div>
  {error&&<p className={styles.toastError} role="alert">{error}</p>}
  <div className={styles.actions}><button type="button" onClick={()=>setSelected(first)}>일정 확인 <span aria-hidden="true">→</span></button><button type="button" disabled={busy} onClick={()=>void ack(first)}>{busy?"저장 중…":"확인했어요"}</button></div>
 </aside>,document.body)}
