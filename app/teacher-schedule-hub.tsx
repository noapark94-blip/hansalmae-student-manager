...
type VehicleStudentOption={id:string;name:string;school:string|null;grade:string|null};
type ManualVehicleAssignment={studentId:string;studentName:string;school:string|null;grade:string|null;studentPhone:string|null;residence:string|null;pickupLocation:string|null;dropoffLocation:string|null;guardians:VehicleGuardian[];weekday:number;direction:"pickup"|"dropoff";time:string};
type ManualVehicleData={students:VehicleStudentOption[];assignments:ManualVehicleAssignment[]};
type AutoVehicleRow={studentId:string;studentName:string;school:string|null;grade:string|null;studentPhone:string|null;residence:string|null;pickupLocation:string|null;dropoffLocation:string|null;guardians:VehicleGuardian[];activities:VehicleActivity[];weekday:number;pickupTime:string|null;dropoffTime:string|null;sources:string[];pickupExcluded:boolean;dropoffExcluded:boolean;manualPickup?:boolean;manualDropoff?:boolean};
type HubData = { teachers: Named[]; students: Named[]; classes: Named[]; classSchedules: ClassSchedule[]; corrections: Correction[]; correctionExceptions: CorrectionException[]; vehicles: VehicleRun[];vehicleExceptions:VehicleException[];todayVehicles:VehicleRun[];autoVehicles:AutoVehicleRow[];vehicleStudents:VehicleStudentOption[] };
...
const emptyData: HubData = { teachers: [], students: [], classes: [], classSchedules: [], corrections: [], correctionExceptions: [], vehicles: [],vehicleExceptions:[],todayVehicles:[],autoVehicles:[],vehicleStudents:[] };
...
export function TeacherScheduleHub({ supabase, profile, initialTab }: { supabase: SupabaseClient; profile: Profile; initialTab: HubTab }) {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    setError("");
    const [{data:result,error:loadError},{data:vehicleResult,error:vehicleError},{data:rosterResult,error:rosterError},{data:autoVehicleResult,error:autoVehicleError},{data:manualVehicleResult,error:manualVehicleError}]=await Promise.all([supabase.rpc("staff_schedule_hub"),supabase.rpc("staff_vehicle_operations"),supabase.rpc("staff_class_schedule_rosters"),supabase.rpc("staff_auto_vehicle_schedule"),supabase.rpc("staff_manual_vehicle_data")]);
    if (loadError || !result||vehicleError||!vehicleResult||rosterError||autoVehicleError||manualVehicleError) setError("시간표 데이터를 불러오지 못했습니다.");
    else { const manual=(manualVehicleResult??{students:[],assignments:[]}) as ManualVehicleData;const hub={ ...emptyData, ...(result as Partial<HubData>),...(vehicleResult as Partial<HubData>),autoVehicles:mergeVehicleRows((autoVehicleResult??[]) as AutoVehicleRow[],manual.assignments??[]),vehicleStudents:manual.students??[] }; const rosterMap=new Map(((rosterResult??[]) as {classId:string;students:Named[]}[]).map((item)=>[item.classId,item.students])); setData({...hub,classSchedules:hub.classSchedules.map((item)=>({...item,students:rosterMap.get(item.classId)??[]}))}); }
    setLoading(false);
  }, [supabase]);
...
function ClassScheduleBoard({ rows, teachers, teacherId, setTeacherId, personal, onAdd, onEdit,onRoster }: { rows: ClassSchedule[]; teachers: Named[]; teacherId: string; setTeacherId: (id: string) => void; personal: boolean; onAdd: () => void; onEdit: (row: ClassSchedule) => void;onRoster:(row:ClassSchedule)=>void }) {
  ...
  const filteredRows=personal||subjectFilter==="전체"?rows:rows.filter((row)=>scheduleSubject(row.subject)===subjectFilter);
  const groups=Array.from(new Map(filteredRows.map((row)=>[`${row.startTime.slice(0,5)}-${row.endTime.slice(0,5)}`,{start:row.startTime.slice(0,5),end:row.endTime.slice(0,5)}])).values()).sort((a,b)=>a.start.localeCompare(b.start)||a.end.localeCompare(b.end));
  const[teacherMenuOpen,setTeacherMenuOpen]=useState(false);
  const selectedTeacher=teachers.find((teacher)=>teacher.id===teacherId)??teachers[0];
  const subjectOptions=scheduleSubjects.map((subject)=>{const subjectRows=subject==="전체"?rows:rows.filter((row)=>scheduleSubject(row.subject)===subject);return{subject,count:new Set(subjectRows.map((row)=>row.classId)).size,color:subjectRows[0]?.color??"#9a8e93"}});
  return <section className="panel hub-panel"><HubToolbar title={personal ? "선생님별 클래스 배정" : "학원 전과목 시간표"} description="요일이 달라도 같은 시간대는 한 줄에, 요일과 시간이 모두 같으면 같은 칸에 함께 표시됩니다."><>{personal&&<div className="teacher-picker" onBlur={(event)=>{if(!event.currentTarget.contains(event.relatedTarget))setTeacherMenuOpen(false)}}><button type="button" className={`teacher-picker-trigger${teacherMenuOpen?" open":""}`} aria-haspopup="listbox" aria-expanded={teacherMenuOpen} onClick={()=>setTeacherMenuOpen((open)=>!open)} onKeyDown={(event)=>{if(event.key==="Escape")setTeacherMenuOpen(false)}}><span><small>담당 선생님</small><b>{selectedTeacher?.name??"선생님 선택"}</b></span><i aria-hidden="true">⌄</i></button>{teacherMenuOpen&&<div className="teacher-picker-menu" role="listbox" aria-label="선생님 선택">{teachers.map((teacher)=><button type="button" role="option" aria-selected={teacher.id===teacherId} className={teacher.id===teacherId?"selected":""} key={teacher.id} onClick={()=>{setTeacherId(teacher.id);setTeacherMenuOpen(false)}}><i>{teacher.name.slice(0,1)}</i><span>{teacher.name}</span>{teacher.id===teacherId?<b>✓</b>:null}</button>)}</div>}</div>}<button className="primary hub-add" onClick={onAdd}>＋ 수업 배정</button></></HubToolbar>{!personal&&<nav className="schedule-subject-filter" aria-label="시간표 과목 필터">{subjectOptions.map((item)=><button type="button" key={item.subject} disabled={item.subject!=="전체"&&item.count===0} className={subjectFilter===item.subject?"active":""} style={{"--subject-color":item.color} as CSSProperties} onClick={()=>setSubjectFilter(item.subject)}><span>{item.subject}</span><em>{item.count}개</em></button>)}</nav>}{filteredRows.length>0&&<div className="aligned-schedule"><div className="aligned-schedule-head"><b>시간</b>{weekdays.map((day)=><b key={day}>{day}</b>)}</div>{groups.map((group)=><div className="aligned-schedule-row" key={`${group.start}-${group.end}`}><strong><time>{group.start}–{group.end}</time></strong>{weekdays.map((day,dayIndex)=>{const cellRows=filteredRows.filter((row)=>row.weekday===dayIndex+1&&row.startTime.slice(0,5)===group.start&&row.endTime.slice(0,5)===group.end);return <div className={`aligned-schedule-cell${cellRows.length>1?" multi":""}`} key={day}>{cellRows.map((row)=><article className="aligned-class-card" key={row.id} style={{"--schedule-color":row.color} as CSSProperties}><button className="aligned-class-main" title={`${row.className} 학생 명단 보기`} onClick={()=>onRoster(row)}><b>{row.className}</b><small>{row.subject} · {row.teachers.map((teacher)=>teacher.name).join("·")||"담당 미배정"}{row.room?` · ${row.room}`:""} · {row.students.length}명</small></button><button className="aligned-class-edit" aria-label={`${row.className} 수업 배정 수정`} onClick={()=>onEdit(row)}>수정</button></article>)}</div>})}</div>)}</div>}{filteredRows.length===0&&<Empty text={subjectFilter==="전체"?"등록된 클래스 시간 배정이 없습니다.":`${subjectFilter} 과목에 등록된 시간 배정이 없습니다.`}/>}</section>;
}
...
function mergeVehicleRows(autoRows:AutoVehicleRow[],manualRows:ManualVehicleAssignment[]){
  const merged=new Map<string,AutoVehicleRow>();
  autoRows.forEach((row)=>merged.set(`${row.studentId}-${row.weekday}`,{...row,manualPickup:false,manualDropoff:false}));
  manualRows.forEach((manual)=>{const key=`${manual.studentId}-${manual.weekday}`;const current=merged.get(key)??{studentId:manual.studentId,studentName:manual.studentName,school:manual.school,grade:manual.grade,studentPhone:manual.studentPhone,residence:manual.residence,pickupLocation:manual.pickupLocation,dropoffLocation:manual.dropoffLocation,guardians:manual.guardians??[],activities:[],weekday:manual.weekday,pickupTime:null,dropoffTime:null,sources:[],pickupExcluded:false,dropoffExcluded:false,manualPickup:false,manualDropoff:false};const sources=Array.from(new Set([...current.sources,"직접 추가"]));merged.set(key,manual.direction==="pickup"?{...current,pickupTime:manual.time,pickupExcluded:false,manualPickup:true,sources}:{...current,dropoffTime:manual.time,dropoffExcluded:false,manualDropoff:true,sources});});
  return Array.from(merged.values()).sort((a,b)=>a.weekday-b.weekday||(a.pickupTime??a.dropoffTime??"").localeCompare(b.pickupTime??b.dropoffTime??"")||a.studentName.localeCompare(b.studentName,"ko"));
}

function VehicleBoard({rows,studentOptions,supabase,canEdit,onChanged}:{rows:AutoVehicleRow[];studentOptions:VehicleStudentOption[];supabase:SupabaseClient;canEdit:boolean;onChanged:()=>Promise<void>}){
  ...
  const renderStudent=(row:AutoVehicleRow,direction:"pickup"|"dropoff")=>{
    ...
    const removeManual=async()=>{setSaving(key);setError("");const time=(direction==="pickup"?row.pickupTime:row.dropoffTime)?.slice(0,5)??"16:00";const{error:removeError}=await supabase.rpc("staff_save_manual_vehicle_assignment",{p_student_id:row.studentId,p_weekday:selectedDay,p_direction:direction,p_time:time,p_remove:true});if(removeError){setError("직접 배정을 삭제하지 못했습니다.");setSaving("");return}await onChanged();setSaving("");};
    return <article className="auto-vehicle-student" key={key}><div><span><button type="button" className="auto-vehicle-student-name" onClick={()=>setSelectedStudent(row)}>{row.studentName}</button>{manual?<em>직접 추가</em>:null}{canEdit?<button type="button" className="auto-vehicle-student-action" disabled={saving===key} onClick={()=>void(manual?removeManual():setExcluded(row,direction,true))}>{manual?"직접 배정 삭제":"차량 제외"}</button>:null}</span><small>{[row.school,row.grade,row.sources.filter((source)=>source!=="직접 추가").join("·")].filter(Boolean).join(" · ")}</small></div></article>;
  };
  return <section className="panel hub-panel auto-vehicle-board">
    <HubToolbar title="차량 운행표" description={canEdit?"정규수업과 첨삭 일정을 합쳐 자동 표시합니다. 관리자와 실장님이 수정할 수 있습니다.":"정규수업과 첨삭 일정을 합쳐 자동 표시합니다. 차량 정보는 조회만 가능합니다."}>{canEdit?<><button type="button" className="primary" onClick={()=>setManualOpen(true)}>＋ 학생 직접 추가</button><button type="button" className={showExcluded?"secondary-button active":"secondary-button"} onClick={()=>setShowExcluded(value=>!value)}>제외 학생 {excludedCount}명</button></>:<></>}</HubToolbar>
    ...
    <section className="auto-vehicle-memo"><header><div><b>{weekdays[selectedDay-1]}요일 운행 메모</b><span>{canEdit?"차량 운행 시 함께 확인할 전달사항을 자유롭게 적어 주세요.":"관리자 또는 실장님이 작성한 전달사항입니다."}</span></div>{noteMessage?<em>{noteMessage}</em>:null}</header><textarea value={notes[selectedDay]??""} disabled={!canEdit||noteLoading||noteSaving} onChange={(event)=>{setNotes(current=>({...current,[selectedDay]:event.target.value}));setNoteMessage("")}} placeholder="예: 19시 하원 차량은 정문 공사로 후문에서 출발 / 우산 확인" rows={6}/>{canEdit?<footer><button type="button" className="primary" disabled={noteLoading||noteSaving} onClick={()=>void saveNote()}>{noteSaving?"저장 중…":"메모 저장"}</button></footer>:null}</section>
    ...
    {selectedStudent?<VehicleStudentDetailModal row={selectedStudent} supabase={supabase} canEdit={canEdit} onSaved={onChanged} onClose={()=>setSelectedStudent(null)}/>:null}
    {canEdit&&manualOpen?<ManualVehicleAssignmentModal studentOptions={studentOptions} initialDay={selectedDay} supabase={supabase} onSaved={async()=>{await onChanged();setManualOpen(false)}} onClose={()=>setManualOpen(false)}/>:null}
  </section>;
}

function ManualVehicleAssignmentModal({studentOptions,initialDay,supabase,onSaved,onClose}:{studentOptions:VehicleStudentOption[];initialDay:number;supabase:SupabaseClient;onSaved:()=>Promise<void>;onClose:()=>void}){
  ...
  const filtered=studentOptions.filter((student)=>!normalized||[student.name,student.school,student.grade].filter(Boolean).some((value)=>value!.toLocaleLowerCase("ko").includes(normalized))).slice(0,30);
  const selected=studentOptions.find((student)=>student.id===studentId);
  const submit=async(event:FormEvent)=>{event.preventDefault();if(!studentId)return setError("추가할 학생을 선택해 주세요.");setSaving(true);setError("");const{error:saveError}=await supabase.rpc("staff_save_manual_vehicle_assignment",{p_student_id:studentId,p_weekday:weekday,p_direction:direction,p_time:time,p_remove:false});if(saveError){setError("차량 직접 배정을 저장하지 못했습니다.");setSaving(false);return}await onSaved();};
  return <div className="modal-backdrop nested" onMouseDown={(event)=>{if(event.target===event.currentTarget)onClose();}}><section className="student-modal manual-vehicle-modal" role="dialog" aria-modal="true" aria-labelledby="manual-vehicle-title"><header><div><p className="eyebrow">차량 직접 배정</p><h2 id="manual-vehicle-title">학생 직접 추가</h2><span>자동 수업 일정은 유지하고 선택한 등원·하원 시간만 직접 지정합니다.</span></div><button type="button" aria-label="닫기" onClick={onClose}>×</button></header><form onSubmit={submit}><label className="manual-vehicle-search"><b>학생 이름 검색</b><input autoFocus value={query} onChange={(event)=>setQuery(event.target.value)} placeholder="학생 이름, 학교 또는 학년 검색"/></label><div className="manual-vehicle-results">{filtered.length?filtered.map((student)=><button type="button" key={student.id} className={studentId===student.id?"selected":""} onClick={()=>setStudentId(student.id)}><i>{student.name.slice(0,1)}</i><span><b>{student.name}</b><small>{[student.school,student.grade].filter(Boolean).join(" · ")||"학교·학년 미입력"}</small></span><em>{studentId===student.id?"선택됨":"선택"}</em></button>):<p>검색 결과가 없습니다.</p>}</div><div className="manual-vehicle-options"><label><b>요일</b><select value={weekday} onChange={(event)=>setWeekday(Number(event.target.value))}>{weekdays.slice(0,5).map((day,index)=><option key={day} value={index+1}>{day}요일</option>)}</select></label><label><b>구분</b><select value={direction} onChange={(event)=>setDirection(event.target.value as "pickup"|"dropoff")}><option value="pickup">등원</option><option value="dropoff">하원</option></select></label><label><b>시간</b><select value={time} onChange={(event)=>setTime(event.target.value)}>{["16:00","17:30","19:00","20:30"].map((slot)=><option key={slot}>{slot}</option>)}</select></label></div>{selected?<p className="manual-vehicle-summary"><b>{selected.name}</b> · {weekdays[weekday-1]}요일 {time} {direction==="pickup"?"등원":"하원"}에 직접 추가됩니다.</p>:null}{error?<p className="form-error">{error}</p>:null}<footer><button className="primary" disabled={saving||!studentId}>{saving?"저장 중…":"직접 배정 저장"}</button></footer></form></section></div>;
}

function VehicleStudentDetailModal({row,supabase,canEdit,onSaved,onClose}:{row:AutoVehicleRow;supabase:SupabaseClient;canEdit:boolean;onSaved:()=>Promise<void>;onClose:()=>void}){
  ...
  const[pickupLocation,setPickupLocation]=useState(row.pickupLocation??"");
  const[dropoffLocation,setDropoffLocation]=useState(row.dropoffLocation??"");
  ...
  const[guardianPhone,setGuardianPhone]=useState(primaryGuardian?.phone??"");
  const[vehicleNote,setVehicleNote]=useState("");
  const[loadingNote,setLoadingNote]=useState(true);
  const[saving,setSaving]=useState(false);
  const[error,setError]=useState("");
  useEffect(()=>{let active=true;void Promise.all([supabase.from("students").select("vehicle_note").eq("id",row.studentId).single(),supabase.from("student_guardians").select("guardian_id,is_primary").eq("student_id",row.studentId).order("is_primary",{ascending:false}).limit(1).maybeSingle()]).then(([noteResult,guardianResult])=>{if(!active)return;if(noteResult.error)setError("차량 특이사항을 불러오지 못했습니다.");else setVehicleNote(noteResult.data?.vehicle_note??"");if(!guardianResult.error&&guardianResult.data?.guardian_id)setGuardianId(guardianResult.data.guardian_id);setLoadingNote(false);});return()=>{active=false};},[row.studentId,supabase]);
  const submit=async(event:FormEvent)=>{event.preventDefault();setSaving(true);setError("");const{error:saveError}=await supabase.from("students").update({school:school.trim()||null,grade:grade.trim()||null,phone:studentPhone.trim()||null,residence:residence.trim()||null,vehicle_pickup_location:pickupLocation.trim()||null,vehicle_dropoff_location:dropoffLocation.trim()||null,vehicle_note:vehicleNote.trim()||null}).eq("id",row.studentId);if(saveError){setError("차량 학생 정보를 저장하지 못했습니다.");setSaving(false);return}if(guardianId!=="new"){const{error:guardianError}=await supabase.from("guardians").update({name:guardianName.trim()||primaryGuardian?.name||`${row.studentName} 보호자`,phone:guardianPhone.trim()}).eq("id",guardianId);if(guardianError){setError("보호자 정보를 저장하지 못했습니다.");setSaving(false);return}}else if(guardianName.trim()||guardianPhone.trim()){if(!guardianPhone.trim()){setError("보호자 연락처를 입력해 주세요.");setSaving(false);return}const{data:newGuardian,error:guardianError}=await supabase.from("guardians").insert({name:guardianName.trim()||`${row.studentName} 보호자`,phone:guardianPhone.trim()}).select("id").single();if(guardianError||!newGuardian){setError("보호자 정보를 저장하지 못했습니다.");setSaving(false);return}const{error:linkError}=await supabase.from("student_guardians").insert({student_id:row.studentId,guardian_id:newGuardian.id,relationship:"학부모",is_primary:true});if(linkError){setError("학생과 보호자 정보를 연결하지 못했습니다.");setSaving(false);return}}await onSaved();onClose();};
  return <div className="modal-backdrop nested" onMouseDown={(event)=>{if(event.target===event.currentTarget)onClose();}}><section className="student-modal vehicle-student-detail-modal" role="dialog" aria-modal="true" aria-labelledby="vehicle-student-detail-title"><header><div><p className="eyebrow">차량 학생 정보</p><h2 id="vehicle-student-detail-title">{row.studentName}</h2><span>기존 학생정보와 연동되며 여기에서 바로 수정할 수 있습니다.</span></div><button type="button" aria-label="닫기" onClick={onClose}>×</button></header><form className="vehicle-student-detail-body" onSubmit={submit} style={{pointerEvents:canEdit?"auto":"none",opacity:canEdit?1:0.82}}><div className="vehicle-student-edit-grid"><label>학교<input value={school} onChange={(event)=>setSchool(event.target.value)} placeholder="학교 미입력"/></label><label>학년<input value={grade} onChange={(event)=>setGrade(event.target.value)} placeholder="학년 미입력"/></label><label>거주지<input value={residence} onChange={(event)=>setResidence(event.target.value)} placeholder="거주지 미입력"/></label><label>학생 연락처<input value={studentPhone} onChange={(event)=>setStudentPhone(event.target.value)} placeholder="학생 연락처 미입력"/></label><label>승차 위치<input value={pickupLocation} onChange={(event)=>setPickupLocation(event.target.value)} placeholder="승차 위치 미입력"/></label><label>하차 위치<input value={dropoffLocation} onChange={(event)=>setDropoffLocation(event.target.value)} placeholder="하차 위치 미입력"/></label><label>보호자 성함<input value={guardianName} disabled={!guardianId} onChange={(event)=>setGuardianName(event.target.value)} placeholder="보호자 미입력"/></label><label>보호자 연락처<input value={guardianPhone} disabled={!guardianId} onChange={(event)=>setGuardianPhone(event.target.value)} placeholder="보호자 연락처 미입력"/></label></div><section className="vehicle-student-schedule"><header><b>{weekdays[row.weekday-1]}요일 정규수업·첨삭</b></header>{activities.length?<div>{activities.map((activity,index)=><article key={`${activity.type}-${activity.startTime}-${index}`}><span className={activity.type}>{activity.type==="class"?"정규수업":"첨삭"}</span><div><b>{activity.title}</b><small>{activity.startTime.slice(0,5)}–{activity.endTime.slice(0,5)}{activity.subject?` · ${activity.subject}`:""}</small></div></article>)}</div>:<p>등록된 일정이 없습니다.</p>}</section><label className="vehicle-note-field"><b>차량 특이사항</b><textarea rows={4} value={vehicleNote} disabled={loadingNote} onChange={(event)=>setVehicleNote(event.target.value)} placeholder="차량 이용 시 확인할 특이사항을 입력해 주세요."/></label>{error?<p className="form-error">{error}</p>:null}<div className="vehicle-detail-save"><button className="primary" disabled={!canEdit||saving||loadingNote}>{canEdit?(saving?"저장 중…":"저장"):"조회 전용"}</button></div></form></section></div>;
}
