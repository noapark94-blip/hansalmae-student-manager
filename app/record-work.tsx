...
function Active({supabase,profile,children}:{supabase:SupabaseClient;profile:Profile;children:ReactNode}){
 ...
 useEffect(()=>{
  if(selected?.origin!=='alimtalk'||selected.source==='correction')return;
  const root=dialog.current;if(!root)return;let done=false;let timer:number|undefined;
  const locate=()=>{if(done)return;const row=root.querySelector<HTMLElement>(`[data-record-student="${selected.studentId}"]`);if(!row)return;done=true;
   const toggle=row.querySelector<HTMLButtonElement>('.mobile-student-record-toggle[aria-expanded="false"]');toggle?.click();
   timer=window.setTimeout(()=>{const field=selected.focusSection==='exam'?row.querySelector<HTMLElement>('.learning-exam-list,.learning-exam'):selected.focusSection==='homework'?row.querySelector<HTMLElement>('.learning-homework.assigned'):null;const target=field??row;target.scrollIntoView({behavior:'smooth',block:'center'});target.animate([{backgroundColor:'#f5dfeb'},{backgroundColor:'transparent'}],{duration:2200});},750);
  };
  const observer=new MutationObserver(locate);observer.observe(root,{childList:true,subtree:true});locate();return()=>{observer.disconnect();if(timer)clearTimeout(timer)};
 },[selected]);
 return <Context.Provider value={{today,items,loading,error,refresh,open:setSelected,profile,supabase}}>{children}
 {toast&&!listOpen&&!selected&&createPortal(<aside className={styles.toast} role="status">
  <span className={styles.toastIcon} aria-hidden="true"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"><path d="M9 5H6a2 2 0 0 0-2 2v13a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2h-3"/><rect x="9" y="2" width="6" height="6" rx="2"/><path d="m8 14 3 3 5-6"/></svg></span>
  <div className={styles.toastContent}><small>{toast.requested?'원장님 기록 요청':'오늘의 기록 마무리'}</small><strong>남은 기록 <span>{toast.count}건</span></strong><p>담당 수업의 기록을 마무리해 주세요.</p></div>
  <button type="button" className={styles.toastClose} aria-label="기록 알림 닫기" onClick={()=>setToast(null)}><svg aria-hidden="true" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round"><path d="m6 6 12 12M18 6 6 18"/></svg></button>
  <div className={styles.toastFooter}><span>정규 · 첨삭 · 보강 · 추가</span><button type="button" className={styles.toastAction} onClick={()=>{setToast(null);setListOpen(true);}}>기록 확인 <span aria-hidden="true">→</span></button></div>
 </aside>,document.body)}
 {(selected||listOpen)&&createPortal(<div className={styles.backdrop}><section ref={dialog} tabIndex={-1} className={styles.dialog} role="dialog" aria-modal="true" aria-label={selected?.origin==='alimtalk'?'알림톡 원본 기록':'기록 마무리'}><header><div><small>{selected?.origin==='alimtalk'?'알림톡 원본 · 현재 저장된 기록':'기록 마무리'}</small><h2>{selected?`${selected.studentName} · ${selected.kind}`:'내 남은 기록'}</h2>{selected&&<p>{selected.date} · {selected.time??'시간 미정'} · {selected.title}</p>}</div><button onClick={()=>void close()} aria-label="기록 마무리 닫기">닫기 ×</button></header><div className={styles.editor}>
 {!selected?<RecordWorkPanel expanded allDates/>:selected.source==='correction'?<Correction key={selected.key} supabase={supabase} initialDate={selected.date} focusStudentId={selected.studentId} focusAssignmentId={selected.origin==='alimtalk'?selected.assignmentId??undefined:undefined} focusTime={selected.origin==='alimtalk'?selected.time??undefined:undefined}/>:selected.sessionId?<Special key={selected.key} embedded supabase={supabase} profile={profile} sessionId={selected.sessionId} focusStudentId={selected.origin==='alimtalk'?selected.studentId:undefined} lessonKind={selected.kind.includes('보강')?'makeup':'additional'} onClose={()=>void close()} onAttendanceChange={()=>refresh()}/>:selected.classId?<Regular key={selected.key} supabase={supabase} profile={profile} lessonTarget={{classId:selected.classId,date:selected.date,requestId:1}}/>:<p>수업 연결 정보를 찾을 수 없습니다.</p>}
 </div></section></div>,document.body)}
 </Context.Provider>;
}
export function useRecordWorkOpen(){return useContext(Context)?.open;}
