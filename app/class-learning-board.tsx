...
export function ClassLearningBoard({
  supabase,
  classId,
  date,
  students,
  ...
  const [mobileCommonCard, setMobileCommonCard] = useState<
  ...
  useEffect(() => {
    let active = true;
    void supabase
      .rpc("staff_class_lesson_state", { p_class_id: classId, p_date: date })
      .then(({ data }) => {
        if (active)
          setLessonState(data === "completed" ? "completed" : "draft");
      });
    return () => {
      active = false;
    };
  ...
  useEffect(() => {
    ...
    if (previous?.classList.contains("class-day-notice"))
  ...
  useEffect(() => {
    ...
    void supabase
      .rpc("staff_class_makeup_options", { p_class_id: classId, p_date: date })
      .then(({ data, error: makeupError }) => {
        if (!active) return;
        if (makeupError) {
          setError(makeupError.message);
          setMakeupOptions([]);
    ...
    void Promise.all([
      ...
      ([
        ...
        const examsByStudent = new Map(
          examRows.map((item) => [item.studentId, item]),
        );
        const homeworkByStudent = new Map(
          ((homeworkResponse.data ?? []) as HomeworkResult[]).map((item) => [
            item.studentId,
            item,
          ]),
        );
  ...
  const activateMakeupDay = async () => {
    ...
    if (!ids.length) {
      setError("현재 등록된 수강생이 없어 보강 수업을 만들 수 없습니다.");
      return;
    }
    if (
      !(await appConfirm({
        eyebrow: "보강 수업 등록",
        title: `${date}을 보강 수업일로 등록할까요?`,
        copy: `현재 수강생 ${ids.length}명이 수업 기록 대상에 추가됩니다.`,
        confirmLabel: "보강 등록",
      }))
    )
      return;
    ...
    const { error: saveError } = await supabase.rpc(
      "staff_save_class_makeup_students",
      { p_class_id: classId, p_date: date, p_student_ids: ids },
    );
    if (saveError) {
  ...
  const deactivateMakeupDay = async () => {
    ...
    if (
      !(await appConfirm({
        eyebrow: "보강 수업 해제",
        title: `${date} 보강 수업을 해제할까요?`,
        notice:
          "출결·수업 내용·숙제·시험 등 실제 기록이 있으면 해제되지 않습니다.",
        confirmLabel: "보강 해제",
        tone: "danger",
      }))
    )
    ...
    const { error: clearError } = await supabase.rpc(
      "staff_clear_empty_class_makeup_day",
  ...
  const updateExam = (studentId: string, patch: Partial<ExamDraft>) =>
    setRows((current) =>
      current.map((row) =>
        row.id === studentId
          ? { ...row, exams: [{ ...row.exams[0], ...patch }] }
          : row,
      ),
    );
  const applyHomework = () => {
        ...
        exams: [
          {
            ...
            examTitle: commonExamTitle,
            maxScore: commonExamMax || "100",
  ...
  const loadPreviousTemplate = async () => {
    ...
    const { data, error: templateError } = await supabase.rpc(
      "staff_class_previous_learning_template",
      { p_class_id: classId, p_date: date },
    );
    if (templateError) {
      setError(
        "이전 수업 기록을 불러오지 못했습니다. 072 migration 적용 여부를 확인해 주세요.",
      );
      setTemplateLoading(false);
      return;
    }
    if (!data) {
      setError("이전에 저장된 수업 기록이 없습니다.");
      setTemplateLoading(false);
      return;
    }
    const template = data as PreviousTemplate;
    setLessonContent(template.lessonContent ?? "");
    setNotice(template.notice ?? "");
    setCommonHomework(template.assignedHomework ?? "");
    setCommonExamType(template.exam?.examType ?? "");
    setCommonExamTitle(template.exam?.examTitle ?? "");
    setCommonExamMax(String(template.exam?.maxScore ?? 100));
      ...
      setRows((current) =>
        current.map((row) => ({
          ...
          exams: [
            {
              ...row.exams[0],
              examType: template.exam.examType ?? "",
              examTitle: template.exam.examTitle ?? "",
              maxScore: String(template.exam.maxScore ?? 100),
              evaluation: template.exam.evaluation ?? "",
              score: "",
            },
          ],
        })),
  ...
  const saveAttendance = async (row: Row, status: Status) => {
    ...
    if (status === "late") {
      setAttendanceEditor({
        row,
        status,
        value: String(row.lateMinutes ?? 10),
      });
      setSaving("");
      return;
    }
  ...
  const saveAttendanceDetail = async () => {
    ...
    const { row, status } = attendanceEditor;
    ...
    if (status === "late") {
      ...
      if (!Number.isFinite(late) || late < 1) {
        setError("지각 시간을 1분 이상의 숫자로 입력해 주세요.");
        return;
      }
    } else {
      reason = value;
  ...
  const deleteRecord = async () => {
    if (
      !(await appConfirm({
        eyebrow: "수업 기록 삭제",
        title: "이 수업 기록을 삭제할까요?",
        notice:
          "출결·수업 내용·시험·숙제와 학부모 리포트 반영이 모두 삭제됩니다.",
        confirmLabel: "기록 삭제",
        tone: "danger",
      }))
    )
    ...
    const { error: deleteError } = await supabase.rpc(
      "staff_delete_class_lesson_record",
      { p_class_id: classId, p_date: date },
    );
    if (deleteError) {
    ...
    <section className="class-learning-board" ref={rootRef} spellCheck={false}>
      <header>
        <div>
          <h3>이번 주 수업 기록</h3>
          <p>
            출결·수업내용·시험·숙제를 한 화면에서 기록하고 학부모 학습리포트로
            연결합니다.
          </p>
        </div>
      ...
      <div className="class-week-navigation">
        <button
          type="button"
          aria-label="이전 주"
          onClick={() => onDate(shiftIsoDate(date, -7))}
        >
          ‹
        </button>
        <div className="class-week-strip">
          {week.map((day, index) => (
            <button
              key={day.date}
              className={`${day.date === date ? "active" : ""} ${day.scheduled ? "scheduled" : ""}`}
              aria-current={day.date === date ? "date" : undefined}
              onClick={() => onDate(day.date)}
            >
              <span>{weekdays[index]}</span>
              <b>{+day.date.slice(8)}</b>
              <small className="class-mobile-day-count">
                {day.students.length
                  ? `${day.students.length}명`
                  : day.scheduled
                    ? "출석 전"
                    : "없음"}
              </small>
              <div>
                {day.students.slice(0, 4).map((student) => (
                  <em
                    className={
                      student.status === "excused" ? "absent" : student.status
                    }
                    key={student.id}
                  >
                    {student.name}
      ...
      {!validDay && !makeupEnabled ? (
        <section
          className="learning-common-record"
          style={{ marginBottom: 16 }}
        >
          <div className="learning-common-title">
            <div>
              <small>정규 수업일 아님</small>
              <b>{date} 보강 수업 기록</b>
            </div>
            <button
              type="button"
              className="primary learning-makeup-activate"
              disabled={makeupLoading}
              onClick={() => void activateMakeupDay()}
            >
              {makeupLoading ? "등록 중…" : "+ 보강 수업 기록"}
            </button>
          </div>
          <p
            style={{
          ...
          >
            이 날짜는 정규 수업 요일이 아닙니다. 보강 수업으로 등록하면 현재
            수강생을 대상으로 출결·수업내용·숙제·시험을 평소와 똑같이 기록할 수
            있습니다.
          </p>
          {error ? (
        ...
        <>
          {!validDay && makeupEnabled ? (
            ...
            >
              <span>
                보강 수업일로 등록된 날짜입니다. 아래에서 평소 수업과 동일하게
                기록할 수 있습니다.
              </span>
              <button
                type="button"
                className="danger-button learning-makeup-deactivate"
                disabled={makeupLoading}
                onClick={() => void deactivateMakeupDay()}
              >
                {makeupLoading ? "해제 중…" : "보강 수업 해제"}
              </button>
            </section>
          ) : null}
          <section
            className={`learning-compact-common ${commonOpen ? "open" : "collapsed"}`}
          >
            <div className="learning-compact-heading">
              <button
                type="button"
                className="learning-common-toggle"
                ...
                <strong>
                  {
                    [
                      lessonContent,
                      commonHomework,
                      commonExamTitle || commonExamType,
                      notice,
                    ].filter((value) => value.trim()).length
                  }
                  /4 입력 <i aria-hidden="true">{commonOpen ? "⌃" : "⌄"}</i>
                </strong>
              </button>
              <span>
                <button
                  type="button"
                  className="learning-common-action"
                  disabled={templateLoading}
                  onClick={() => void loadPreviousTemplate()}
                >
                  {templateLoading ? "불러오는 중…" : "지난 수업 불러오기"}
                </button>
                <button
                  type="button"
                  className="learning-common-action"
            ...
            {commonOpen ? (
              <div className="learning-common-grid">
                <section
                  className={`learning-common-card mobile-disclosure ${lessonContent.trim() ? "filled" : ""} ${mobileCommonCard === "lesson" ? "mobile-open" : ""}`}
                >
                  <button
                    type="button"
                    className="learning-common-card-toggle"
                    aria-expanded={mobileCommonCard === "lesson"}
                    onClick={() =>
                      setMobileCommonCard((current) =>
                        current === "lesson" ? null : "lesson",
                      )
                    }
                  >
                    <span>
                    ...
                    <em>{lessonContent.trim() ? "입력됨" : "미입력"}</em>
                    <i aria-hidden="true">⌄</i>
                  </button>
                  <textarea
                    value={lessonContent}
                    onChange={(e) => setLessonContent(e.target.value)}
                    placeholder="교재·단원·핵심 수업 내용"
                    rows={4}
                  />
                  <button
                    type="button"
                    className="learning-apply-button"
                    onClick={applyLessonContent}
                  >
                    전체 적용
                  </button>
                </section>
                <section
                  className={`learning-common-card mobile-disclosure ${commonHomework.trim() ? "filled" : ""} ${mobileCommonCard === "homework" ? "mobile-open" : ""}`}
                >
                  <button
                    type="button"
                    className="learning-common-card-toggle"
                    aria-expanded={mobileCommonCard === "homework"}
                    onClick={() =>
                      setMobileCommonCard((current) =>
                        current === "homework" ? null : "homework",
                      )
                    }
                  >
                    <span>
                    ...
                    <em>{commonHomework.trim() ? "입력됨" : "미입력"}</em>
                    <i aria-hidden="true">⌄</i>
                  </button>
                  <textarea
                    value={commonHomework}
                    onChange={(e) => setCommonHomework(e.target.value)}
                    placeholder="교재·페이지·문제 번호·제출일"
                    rows={4}
                  />
                  <button
                    type="button"
                    className="learning-apply-button"
                    onClick={applyHomework}
                  >
                    전체 적용
                  </button>
                </section>
                <button
                  type="button"
                  className="learning-common-mobile-apply"
                  disabled={!lessonContent.trim() && !commonHomework.trim()}
                  onClick={applyCommonBasics}
                >
                  공통 내용을 전체 학생에게 적용
                </button>
                <section
                  className={`learning-common-card disclosure ${commonExamTitle.trim() || commonExamType.trim() ? "filled" : ""} ${bulkOpen ? "open" : ""}`}
                >
                  <button
                    type="button"
                    className="learning-card-disclosure"
                  ...
                  >
                    ...
                    <em>{commonExamTitle || commonExamType || "미입력"}</em>
                    <strong>{bulkOpen ? "접기" : "입력"}</strong>
                  </button>
                  {bulkOpen ? (
                    <div className="learning-card-exam">
                      <select
                        value={commonExamType}
                        onChange={(e) => setCommonExamType(e.target.value)}
                      >
                      ...
                      <input
                        value={commonExamTitle}
                        onChange={(e) => setCommonExamTitle(e.target.value)}
                        placeholder="시험명·범위"
                      />
                      <span className="learning-common-score">
                        <input
                          value=""
                          readOnly
                          aria-label="원점수는 학생별 입력"
                          placeholder="학생별 입력"
                        />
                        <em>/</em>
                        <input
                          value={commonExamMax}
                          onChange={(e) => setCommonExamMax(e.target.value)}
                          inputMode="decimal"
                          placeholder="만점"
                        />
                      </span>
                      <input
                        value={commonEvaluation}
                        onChange={(e) => setCommonEvaluation(e.target.value)}
                        placeholder="평가·피드백"
                      />
                      <button
                        type="button"
                      ...
                      <small>
                        종류·시험명·만점·평가를 전체 학생에게 적용하고 원점수만
                        학생별로 입력합니다.
                      </small>
                    </div>
                  ) : (
                ...
                <section
                  className={`learning-common-card disclosure ${notice.trim() ? "filled" : ""} ${noticeOpen ? "open" : ""}`}
                >
                  <button
                    type="button"
                    className="learning-card-disclosure"
                  ...
                  >
                    ...
                    <em>{notice || "미입력"}</em>
                    <strong>{noticeOpen ? "접기" : "입력"}</strong>
                  </button>
                  {noticeOpen ? (
                    <textarea
                      value={notice}
                      onChange={(e) => setNotice(e.target.value)}
                      placeholder="준비물·일정·반 전체 안내"
                      rows={5}
                    />
                  ) : (
                    <p>공지가 있을 때만 입력하세요.</p>
          ...
          <div className="learning-board-heading">
            <span>학생·출결</span>
            <span>개인별 수업 내용</span>
            <span className="learning-exam-heading">
              <b>개인별 시험</b>
              ...
              {rows.map((row) => {
                ...
                const converted =
                  ...
                  max > 0
                    ...
                    : null;
                const detailCount = [
                  row.lessonContent.trim(),
                  exam.examType.trim() || exam.examTitle.trim(),
                  row.inspectionStatus.trim() || row.assignedHomework.trim(),
                ].filter(Boolean).length;
                const mobileOpen = openStudentId === row.id;
                return (
                  <article
                  ...
                  >
                    <div className="mobile-student-record-head">
                      <button
                        type="button"
                        className="mobile-student-record-toggle"
                    ...
                    <div className="learning-person-attendance">
                      <span className="learning-student">
                        <button
                          type="button"
                          className="learning-student-history-button"
                          onClick={() => setHistoryStudent(row)}
                          title={`${row.name} 학생 누적 수업 기록 보기`}
                        >
                          <i>{row.name[0]}</i>
                          <b>
                            {row.name}
                            {row.directAdded ? (
                              <em className="lesson-direct-badge">직접 추가</em>
                            ) : null}
                          </b>
                          <small>
                            {[row.school, row.grade]
                      ...
                      <div className="learning-attendance">
                        ...
                        {row.status === "late" ? (
                          <small>
                            {row.lateMinutes}분 지각 · 같은 버튼을 다시 누르면
                            취소
                          </small>
                        ) : null}
                        {row.status === "absent" ? (
                        ...
                        {row.status === "present" ? (
                          <small>같은 버튼을 다시 누르면 취소</small>
                        ) : null}
                      </div>
                    </div>
                    <div className="learning-individual-content">
                      <textarea
                        value={row.lessonContent}
                        onChange={(e) =>
                          update(row.id, { lessonContent: e.target.value })
                        }
                        placeholder="이 학생의 교재·단원·진도"
                        rows={4}
                      />
                    </div>
                      ...
                      <div className="learning-exam-card">
                        ...
                        <div className="learning-exam individual">
                          <select
                            value={exam.examType}
                            onChange={(e) =>
                              updateExam(row.id, { examType: e.target.value })
                            }
                          >
                            <option value="">종류 선택</option>
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              updateExam(row.id, { examTitle: e.target.value })
                            }
                            placeholder="시험명·범위"
                          />
                          <span>
                            <input
                              ...
                              value={exam.score}
                              onChange={(e) =>
                                updateExam(row.id, { score: e.target.value })
                              }
                              placeholder="원점수"
                            />
                            <em>/</em>
                            <input
                              ...
                              value={exam.maxScore}
                              onChange={(e) =>
                                updateExam(row.id, { maxScore: e.target.value })
                              }
                              placeholder="만점"
                            />
                          </span>
                          <input
                            ...
                            onChange={(e) =>
                              updateExam(row.id, { evaluation: e.target.value })
                            }
                            placeholder="평가·피드백"
                          />
                        </div>
                        <small className="exam-percent">
                          {converted === null
                            ? "점수를 입력하면 100점 환산점수가 표시됩니다."
                            : `원점수 ${exam.score}/${exam.maxScore} · 환산 ${converted}점`}
                        </small>
                      </div>
                    </div>
                    <div className="learning-homework previous">
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          update(row.id, { inspectionStatus: e.target.value })
                        }
                      >
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          update(row.id, { inspectionNote: e.target.value })
                        }
                        placeholder="검사 메모"
                      />
                    </div>
                    <div className="learning-homework assigned">
                      <textarea
                        value={row.assignedHomework}
                        onChange={(e) =>
                          update(row.id, { assignedHomework: e.target.value })
                        }
                        placeholder="교재·페이지·문제 번호·제출일"
                        rows={4}
                      />
      ...
      {attendanceEditor ? (
        <div
          className="modal-backdrop nested attendance-editor-backdrop"
          onMouseDown={(event) => {
            if (event.target === event.currentTarget) setAttendanceEditor(null);
          }}
        >
          <form
            className="attendance-editor-modal"
            role="dialog"
            aria-modal="true"
            aria-labelledby="attendance-editor-title"
            onSubmit={(event) => {
              event.preventDefault();
              void saveAttendanceDetail();
            }}
          >
            <header>
              <span
                className={`attendance-editor-icon ${attendanceEditor.status}`}
              >
                {attendanceEditor.status === "late" ? "분" : "!"}
              </span>
              <div>
                <small>
                  {attendanceEditor.status === "late"
                    ? "지각 시간 기록"
                    : "결석 사유 기록"}
                </small>
                <h2 id="attendance-editor-title">
                  {attendanceEditor.row.name} 학생
                </h2>
              </div>
              <button
                type="button"
                aria-label="닫기"
                onClick={() => setAttendanceEditor(null)}
              >
                ×
              </button>
            </header>
            <label>
              ...
              {attendanceEditor.status === "late" ? (
                <div className="attendance-minute-input">
                  <input
                    autoFocus
                    type="number"
                    min="1"
                    inputMode="numeric"
                    value={attendanceEditor.value}
                    onChange={(event) =>
            ...
            <footer>
              ...
              <button
                type="submit"
                className="primary"
                disabled={saving === attendanceEditor.row.id}
              >
                {saving === attendanceEditor.row.id ? "저장 중…" : "기록하기"}
              </button>
            </footer>
          </form>
        </div>
      ...
      {historyStudent ? (
        ...
        >
          <section
            className="student-modal student-learning-history-modal"
            role="dialog"
            aria-modal="true"
          >
            <header>
              <div>
                <p className="eyebrow">교직원 전용 · 누적 수업 기록</p>
                <h2>
                  {historyStudent.name}
                  <small className="history-type-label">클래스 수업 기록</small>
                </h2>
                <span>
                  {[historyStudent.school, historyStudent.grade]
                    .filter(Boolean)
                    .join(" · ") || "학생 기록"}
                </span>
              </div>
...
function FamilyReportReadStatus({
  supabase,
  classId,
  date,
}: {
  ...
  const load = useCallback(async () => {
    ...
    const { data: next, error } = await supabase.rpc(
      "staff_class_family_report_read_status",
      { p_class_id: classId, p_date: date },
    );
    if (error) {
      setAvailable(false);
  ...
  const confirmed = data?.confirmedStudents ?? 0,
    unconfirmed = data?.unconfirmedStudents ?? 0,
    unlinked = data?.unlinkedStudents ?? 0;
  const oldUnconfirmed =
    !!data?.lessonId && unconfirmed > 0 && daysAgo(date) >= 2;
  return (
    <section
      className={`family-read-status ${oldUnconfirmed ? "needs-attention" : ""}`}
    >
      <button
        type="button"
        className="family-read-summary"
        onClick={() => setOpen((v) => !v)}
        disabled={loading}
      >
        <span className="family-read-title">
          <small>학부모 확인 현황</small>
          <b>
            {!data?.lessonId ? (
              "리포트 생성 전"
            ) : (
              <>
                확인 <em>{confirmed}</em> / {data.totalStudents}명
              </>
            )}
          </b>
          {oldUnconfirmed ? (
            <em className="family-read-warning">2일 이상 미확인</em>
          ) : null}
        </span>
        <span className="family-read-pills">
          {data?.lessonId && unconfirmed ? (
            <em className="unconfirmed">미확인 {unconfirmed}명</em>
          ) : null}
          {unlinked ? <em className="unlinked">미연결 {unlinked}명</em> : null}
        ...
        <div className="family-read-details">
          {data.students.map((student) => (
            <article
              ...
              className={
                oldUnconfirmed && student.status === "unconfirmed"
                  ? "stale-unconfirmed"
                  : ""
              }
            >
                ...
                {student.status === "confirmed"
                  ...
                  : student.status === "unconfirmed"
                    ? oldUnconfirmed
                      ...
                      : "미확인"
                    : "학부모 계정 미연결"}
                {student.status === "confirmed" && student.viewedAt ? (
                  <small>{formatReadTime(student.viewedAt)}</small>
                ) : null}
              </span>
            </article>
          ))}
          {!data.students.length ? (
            <p>이 날짜의 수강 학생이 없습니다.</p>
          ) : null}
          <footer>
            <span>보호자 한 명이라도 확인하면 학부모 확인으로 표시됩니다.</span>
...
export function ExamCategoryModal({
  supabase,
  categories,
  onClose,
  onChanged,
  ...
  const rename = async (category: ExamCategory) => {
    const value = await appPrompt({
      eyebrow: "시험 종류 이름 변경",
      title: `‘${category.name}’의 새 이름을 입력해 주세요`,
      inputLabel: "시험 종류 이름",
    ...
    const { error: changeError } = await supabase.rpc(
      "staff_set_exam_category",
      { p_id: category.id, p_name: value.trim(), p_active: true },
    );
  ...
  const remove = async (category: ExamCategory) => {
    ...
    const { error: deleteError } = await supabase.rpc(
      "staff_set_exam_category",
      { p_id: category.id, p_name: category.name, p_active: false },
    );
...
function Month({
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const months = Array.from(
        new Set(calendar.map((date) => date.slice(0, 7))),
      );
      const responses = await Promise.all(
        months.map((value) =>
          supabase.rpc("staff_class_attendance_calendar", {
            p_class_id: classId,
            p_anchor_date: `${value}-01`,
            p_view: "month",
          }),
        ),
      );
      ...
      if (failed) {
        setError("출석 캘린더를 불러오지 못했습니다.");
        setRecords({});
      } else {
        const next: Record<string, CalendarDay> = {};
        for (const response of responses)
          for (const day of (response.data ?? []) as CalendarDay[])
            next[day.date] = day;
        setRecords(next);
      }
      setLoading(false);
  ...
  return (
    ...
    >
      <section className="student-modal class-month-modal correction-month-modal">
        <header>
          <div>
            <p className="eyebrow">전체 출결 현황</p>
            <h2>전체 출석 캘린더</h2>
            <span>
              월간 수업 예정과 출석·지각·결석 기록을 한눈에 확인합니다.
            </span>
          </div>
        ...
        <nav
          className="correction-month-toolbar"
          aria-label="출석 달력 월 이동"
        >
          <button
            type="button"
            aria-label="이전 달"
          ...
          <button type="button" onClick={() => setMonth(koreaMonth())}>
            이번 달
          </button>
        </nav>
        {error ? <p className="form-error">{error}</p> : null}
        {loading ? (
          <p className="correction-month-loading">
            캘린더를 불러오는 중이에요…
          </p>
        ) : (
          <>
            <div className="correction-month-weekdays">
              {weekdays.map((day) => (
                <b key={day}>{day}</b>
              ))}
            </div>
            <div className="correction-month-grid">
              {calendar.map((date) => {
                ...
                return (
                  <button
                    type="button"
                    key={date}
                    className={`${inMonth ? "" : "outside"} ${date === anchor ? "selected" : ""} ${date === koreaToday() ? "today" : ""} ${day?.scheduled ? "scheduled" : ""}`}
                    onClick={() => {
                      onDate(date);
                      onClose();
                    }}
...
function classCalendarDays(month: string) {
  const first = `${month}-01`,
    start = shiftIsoDate(first, 1 - isoWeekday(first));
  const [yearValue, monthValue] = month.split("-").map(Number);
  const lastDate = new Date(Date.UTC(yearValue, monthValue, 0));
  const last = `${lastDate.getUTCFullYear()}-${String(lastDate.getUTCMonth() + 1).padStart(2, "0")}-${String(lastDate.getUTCDate()).padStart(2, "0")}`,
    end = shiftIsoDate(last, 7 - isoWeekday(last));
  const dates: string[] = [];
  for (let date = start; ; date = shiftIsoDate(date, 1)) {
    dates.push(date);
    if (date === end) break;
  }
  return dates;
