...
export function AlimtalkSendCenter({supabase}:{supabase:SupabaseClient;students:StudentRow[]}){
  ...
  async function sendRow(row:ReadyStudent){const itemPreview=buildPreview(row.studentName,period.start,type,row.lessons);const{error}=await supabase.functions.invoke("send-learning-alimtalk",{body:{studentId:row.studentId,reportType:type,periodStart:period.start,periodEnd:period.end,lessonSummary:itemPreview.lesson,attendanceSummary:itemPreview.attendance,examSummary:itemPreview.exam,homeworkSummary:itemPreview.homework,learningSummary:[itemPreview.exam&&`시험 ${itemPreview.exam}`,itemPreview.homework&&`과제 ${itemPreview.homework}`].filter(Boolean).join("\n")}});if(!error)return null;let text=error.message;const context=(error as{context?:Response}).context;if(context)try{const body=await context.clone().json() as{error?:string};if(body.error)text=body.error}catch{}return text}
  ...
  return <section className={styles.center}>
    ...
    <div className={styles.summary}><div><small>예정 학생</small><b>{readyStudents.length}명</b><span>이 기간에 수업이 있음</span></div><div><small>기록 완료</small><b>{readyStudents.filter(row=>row.complete).length}명</b><span>완료 학생만 자동 선택</span></div><div><small>발송 선택</small><b>{checkedRows.length}명</b><span>체크한 학생만 발송</span></div></div>
    ...
    <div className={styles.bulkBar}><div><b>선택 학생 {checkedRows.length}명</b><span>완료 학생은 자동 선택되며, 미완료 학생은 직접 선택할 수 있습니다.</span></div><button disabled={!checkedRows.length||sending} onClick={()=>setConfirming(true)}>{sending?"일괄 발송 중…":`선택 ${checkedRows.length}명 일괄 발송`}</button></div>
