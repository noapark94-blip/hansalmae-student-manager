...
for(const [error,preserve] of [[{message:'Failed to fetch',code:''},true],[{message:'연결된 자녀만 확인할 수 있습니다.',code:'P0001'},false]])test('grades cached records '+(preserve?'survive transport errors':'are cleared after access denial'),async()=>{const h=harness();const first=h.ctx.run('a');h.answer('a');await first;const next=h.ctx.run('a');h.answer('a',error);await next;assert.equal(Boolean(h.state.data),preserve);assert.equal(Boolean(h.state.academic),preserve);assert.equal(h.state.loading,false);});
