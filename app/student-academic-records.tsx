...
export function StudentAcademicRecords({supabase,studentId}:{supabase:SupabaseClient;studentId:string}) {
  ...
  return <section className="academic-records">
    ...
    </div><footer><small>원점수·등급·석차 중 하나 이상 입력해 주세요.</small><div><button type="button" onClick={()=>setEditorOpen(false)}>취소</button><button className="academic-primary" disabled={saving}>{saving?"저장 중…":"저장"}</button></div></footer></form>:null}
