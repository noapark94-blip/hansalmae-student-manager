...
export function FamilyExamGrowth({supabase,studentId}:{supabase:SupabaseClient;studentId:string}){
  ...
  useEffect(()=>{let active=true;setLoading(true);void supabase.rpc("family_exam_progress",{p_student_id:studentId}).then(({data})=>{if(active){setItems((data??[]) as Exam[]);setLoading(false);}});return()=>{active=false};},[studentId,supabase]);
  ...
  return <section className="panel family-growth-card">
    <header><div><p className="eyebrow">성적 성장</p><h2>최근 시험 추이</h2></div><nav>{subjects.map(name=><button key={name} className={selected===name?"active":""} onClick={()=>setSubject(name)}>{name}</button>)}</nav></header>
