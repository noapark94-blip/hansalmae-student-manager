    ...
    {staff?<div className={styles.workspace}>
      <aside className={styles.controls}>
        <div className={styles.studentFilters}>
          <label className={styles.studentSearch}><span>학생 검색</span><i><HansalmaeIcon name="students" size={16}/><input value={studentQuery} onChange={e=>{setStudentQuery(e.target.value);resetStudentSelection()}} placeholder="이름, 학교, 과목 검색" aria-label="리포트 학생 검색"/>{studentQuery&&<button type="button" onClick={()=>{setStudentQuery("");resetStudentSelection()}} aria-label="학생 검색어 지우기">×</button>}</i></label>
...
function FilterSelect({label,value,onChange,allLabel,options}:{label:string;value:string;onChange:(value:string)=>void;allLabel?:string;options:{value:string;label:string}[]}){return <label><span>{label}</span><select value={value} onChange={event=>onChange(event.target.value)}>{allLabel&&<option value="all">{allLabel}</option>}{options.map(option=><option key={option.value} value={option.value}>{option.label}</option>)}</select></label>}
