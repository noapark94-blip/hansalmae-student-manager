...
type State={items:Reminder[];people:Person[];peopleError:string;retryPeople:()=>void;error:string;loading:boolean;refresh:()=>Promise<void>;ack:(r:Reminder)=>Promise<void>;open:(r:Reminder)=>void};
...
function ActiveProvider({supabase,profile,children}:{supabase:SupabaseClient;profile:Profile;children:ReactNode}){
 ...
 return <Context.Provider value={{items,people,peopleError,retryPeople:()=>setPeopleAttempt(n=>n+1),error,loading,refresh,ack,open:setSelected}}>{children}
...
export function ScheduleReminderList(){const state=useScheduleReminders();if(!state)return null;return <div className={styles.list}><div className={styles.toolbar}><span>최근 30일 · 미확인 {state.items.filter(r=>!r.read).length}건</span><button onClick={()=>void state.refresh()}>새로고침</button></div>{state.error&&<p role="alert">{state.error}</p>}{state.loading?<p>일정 알림을 확인하고 있어요.</p>:!state.items.length?<div className={styles.empty}><b>등록된 일정 알림이 없습니다</b><p>당일 알림을 켠 담당 일정이 여기에 모입니다.</p></div>:state.items.map(r=><button className={`${styles.item} ${r.read?styles.read:""}`} key={key(r)} onClick={()=>state.open(r)}><span>{r.kind}<em>{r.read?"확인 완료":"미확인"}</em></span><b>{r.title}</b><small>{r.date} · {r.time??"시간 미지정"}{r.place?` · ${r.place}`:""}</small>{r.person&&<p>{r.person}</p>}</button>)}</div>;}
