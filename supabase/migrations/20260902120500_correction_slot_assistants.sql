create table if not exists public.correction_slot_assistants (
  ...
  primary key (weekday, start_time, assistant_profile_id)
);

alter table public.correction_slot_assistants enable row level security;
revoke all on table public.correction_slot_assistants from public, anon, authenticated;
...
begin
  ...
  return jsonb_build_object(
    'canManage', public.current_user_role() in ('admin', 'teacher'),
    'assistants', coalesce((
