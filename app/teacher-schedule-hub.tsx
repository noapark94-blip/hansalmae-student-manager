...
type HubData = { teachers: Named[]; students: Named[]; classes: Named[]; classSchedules: ClassSchedule[]; corrections: Correction[]; correctionExceptions: CorrectionException[]; correctionCapacities:CorrectionCapacity[]; vehicles: VehicleRun[] };
type Editor = { kind: "class"; row?: ClassSchedule } | { kind: "correction"; row?: Correction } | { kind: "exception"; row: Correction } | {kind:"capacity"}| { kind: "vehicle"; row?: VehicleRun } | null;

const emptyData: HubData = { teachers: [], students: [], classes: [], classSchedules: [], corrections: [], correctionExceptions: [], correctionCapacities:[], vehicles: [] };
...
function VehicleEditor({ supabase, data, row, onClose, onSaved }: EditorProps & { data: HubData; row?: VehicleRun }) {
  ...
  const submit = async (event: FormEvent) => { event.preventDefault(); if (!managerId || !location.trim()) return setError("차량실장님과 탑승 위치를 입력해 주세요."); setSaving(true); const { error: saveError } = await supabase.rpc("staff_save_vehicle_run", { p_run_id: row?.id ?? null, p_manager_id: managerId, p_weekday: weekday, p_pickup_time: pickupTime, p_pickup_location: location.trim(), p_student_ids: studentIds }); if (saveError) return fail(saveErrorMessage(saveError.message, "운행 정보를 저장하지 못했습니다.")); await onSaved(); };
...
function saveErrorMessage(message: string, fallback: string) { return message.includes("정원") || message.includes("시간 충돌") || message.includes("같은 시간") || message.includes("같은 시각") || message.includes("같은 첨삭") || message.includes("겹치는 수업") || message.includes("이미 배정") || message.includes("이미 있") || message.includes("늦어야") ? message : fallback; }
