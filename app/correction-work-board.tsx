  ...
  const renderRow=(row:Occurrence)=>{
    ...
    return <article key={key} className={`${row.assignment.isDateOverride?"correction-date-override-row ":""}${report.published?"correction-mobile-complete":""}`}>
      ...
      <div className={`correction-mobile-student-details ${detailsOpen?"open":""}`}>
        ...
        <div className="correction-task-cell"><textarea value={report.correctionContent??""} onChange={e=>updateDraft(row,{correctionContent:e.target.value})} placeholder="오늘 실제로 진행한 첨삭 과제·오답·개념 설명 내용을 기록하세요." rows={5}/><div className="correction-task-check"><span>{([['completed','완료'],['partial','부분 완료'],['incomplete','미완료']] as const).map(([value,label])=><button type="button" key={value} className={`${value} ${report.correctionTaskStatus===value?'active':''}`} onClick={()=>updateDraft(row,{correctionTaskStatus:report.correctionTaskStatus===value?null:value})}>{label}</button>)}</span><input value={report.correctionTaskFeedback??""} onChange={e=>updateDraft(row,{correctionTaskFeedback:e.target.value})} placeholder="검사 피드백 (선택)"/></div></div>
      </div>
    </article>;
  };
