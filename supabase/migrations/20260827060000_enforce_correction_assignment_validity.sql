-- 첨삭 고정 배정의 시작일·종료일을 모든 조회에서 일관되게 적용합니다.
-- 기존 첨삭 기록은 보존하고, 일정 변경·종료 이후에도 과거 달력은 당시 명단을 유지합니다.

update public.correction_assignments a
set valid_from=least(
  a.valid_from,
  (timezone('Asia/Seoul',a.created_at))::date,
  coalesce((select min(r.correction_date) from public.correction_reports r where r.assignment_id=a.id),a.valid_from)
)
where a.subject is not null;

create index if not exists correction_assignments_validity_idx
on public.correction_assignments(weekday,valid_from,valid_until)
...
declare result uuid;legacy_teacher uuid;existing public.correction_assignments;v_today date:=(timezone('Asia/Seoul',now()))::date;material_change boolean;
begin
  ...
  if p_id is null then
    ...
    if material_change and existing.valid_from<v_today then
      update public.correction_assignments set active=false,valid_until=greatest(valid_from,v_today-1),updated_at=now() where id=existing.id;
      insert into public.correction_assignments(student_id,teacher_profile_id,weekday,slot_index,valid_from,subject,start_time,end_time,tutor_profile_id,supervisor_profile_id,note,active,created_by,updated_at)
      values(p_student_id,legacy_teacher,p_weekday,null,v_today,p_subject,p_start_time,p_end_time,p_tutor_profile_id,p_supervisor_profile_id,nullif(trim(p_note),''),true,auth.uid(),now()) returning id into result;
    else
      update public.correction_assignments set student_id=p_student_id,teacher_profile_id=legacy_teacher,weekday=p_weekday,slot_index=null,subject=p_subject,start_time=p_start_time,end_time=p_end_time,
...
declare a public.correction_assignments;v_today date:=(timezone('Asia/Seoul',now()))::date;last_record date;
begin
  ...
  if a.valid_from>=v_today and last_record is null then
    delete from public.correction_assignments where id=p_id;
  else
    update public.correction_assignments set active=false,valid_until=greatest(valid_from,coalesce(last_record,v_today-1)),updated_at=now() where id=p_id;
  end if;
end $$;
...
begin
  ...
  return coalesce((select jsonb_agg(jsonb_build_object(
    'studentId',s.id,'rows',coalesce((select jsonb_agg(jsonb_build_object(
    ...
    ) order by a.weekday,a.start_time,a.subject) from public.correction_assignments a
      where a.student_id=s.id and a.active and a.valid_from<=v_today and (a.valid_until is null or a.valid_until>=v_today)),'[]'::jsonb)
  ) order by s.name) from public.students s where s.status in ('active','재원')),'[]'::jsonb);
end $$;

revoke all on function public.correction_day_board(text),public.correction_management_board_v2(text),public.staff_save_correction_assignment(uuid,uuid,text,smallint,time,time,uuid,uuid,text),public.staff_delete_correction_assignment(uuid),public.staff_correction_student_timetables() from public,anon;
grant execute on function public.correction_day_board(text),public.correction_management_board_v2(text),public.staff_save_correction_assignment(uuid,uuid,text,smallint,time,time,uuid,uuid,text),public.staff_delete_correction_assignment(uuid),public.staff_correction_student_timetables() to authenticated;
