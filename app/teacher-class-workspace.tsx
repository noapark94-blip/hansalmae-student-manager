...
function ClassStudentScheduleEditor({supabase,classRoom,student,enrolled,onClose,onSaved}:{supabase:SupabaseClient;classRoom:ClassRoom;student:RosterStudent;enrolled:boolean;onClose:()=>void;onSaved:()=>Promise<void>}){
  ...
  useEffect(()=>{let active=true;void(async()=>{const{data,error:loadError}=await supabase.rpc("staff_class_student_schedule_choices",{p_class_id:classRoom.id,p_student_id:student.id});if(!active)return;if(loadError)setError(loadError.message);else{const rows=((data as Array<{scheduleId:string;weekday:number;startTime:string;endTime:string;assigned:boolean}>)??[]).map((item)=>({id:item.scheduleId,weekday:item.weekday,startTime:item.startTime,endTime:item.endTime,assigned:item.assigned}));setChoices(rows);setSelectedIds(rows.filter((item)=>item.assigned).map((item)=>item.id))}setLoading(false)})();return()=>{active=false}},[classRoom.id,student.id,supabase]);
  ...
  const save=async()=>{if(!selectedIds.length){setError("최소 한 개의 수업 요일·시간을 선택해 주세요.");return}setSaving(true);setError("");const{error:saveError}=await supabase.rpc("staff_save_class_student_schedule_assignments",{p_class_id:classRoom.id,p_student_id:student.id,p_schedule_ids:selectedIds});if(saveError){setError(saveError.message);setSaving(false);return}await onSaved()};
  return <div className="nested-modal-backdrop" onMouseDown={(event)=>{if(event.target===event.currentTarget)onClose()}}><section className="modal class-student-schedule-modal" role="dialog" aria-modal="true" aria-labelledby="class-student-schedule-title"><header><div><p className="eyebrow">요일별 수강 배정</p><h2 id="class-student-schedule-title">{student.name}</h2><p>{classRoom.name}에서 실제로 참석하는 시간만 선택합니다.</p></div><button type="button" className="modal-close" aria-label="닫기" onClick={onClose}>×</button></header><div className="modal-body"><div className="schedule-scope-note"><strong>다른 과목·반은 그대로 유지돼요</strong><span>선택한 시간만 충돌을 확인해 {enrolled?"시간표를 변경":"학생을 추가"}합니다.</span></div>{loading?<p className="empty-state">수업 시간을 불러오는 중이에요…</p>:choices.length?<div className="student-schedule-choice-list">{choices.map((choice)=>{const selected=selectedIds.includes(choice.id);return <button type="button" key={choice.id} className={selected?"selected":""} aria-pressed={selected} onClick={()=>toggleChoice(choice.id)}><i aria-hidden="true">{selected?"✓":""}</i><span><b>{weekdays[choice.weekday-1]}요일</b><small>{choice.startTime.slice(0,5)}–{choice.endTime.slice(0,5)}</small></span></button>})}</div>:<p className="empty-state">먼저 클래스에 수업 요일·시간을 등록해 주세요.</p>}{error?<p className="form-error">{error}</p>:null}</div><footer><button type="button" className="secondary-button" onClick={onClose}>취소</button><button type="button" className="primary" disabled={loading||saving||!choices.length} onClick={()=>void save()}>{saving?"저장 중…":enrolled?`선택한 ${selectedIds.length}개 저장`:`선택한 ${selectedIds.length}개로 추가`}</button></footer></section></div>
}
