-- Monthly targets and read-only coverage. Operational lesson rows are never rewritten.
create table public.student_monthly_lesson_targets (
 student_id uuid not null references public.students(id) on delete cascade,
 subject text not null check(subject in ('국어','영어')),
 month date not null check(month=date_trunc('month',month)::date),
 target integer not null check(target between 0 and 100),
 version integer not null default 1,
 updated_at timestamptz not null default now(),
 primary key(student_id,subject,month)
);
alter table public.student_monthly_lesson_targets enable row level security;
revoke all on public.student_monthly_lesson_targets from public,anon,authenticated;
-- All access goes through the scoped RPCs below.
create function public.admin_save_monthly_lesson_target(p_student uuid,p_subject text,p_month date,p_target integer,p_expected_version integer default null)
returns void language plpgsql security definer set search_path='' as $$
declare m date:=date_trunc('month',p_month)::date; current_version integer;
begin
 if auth.uid() is null or public.current_user_role() is distinct from 'admin' then raise exception '관리자만 월 목표를 조정할 수 있습니다.'; end if;
 if m is null or p_subject not in ('국어','영어') or p_subject is null or p_target is null or p_target not between 0 and 100 then raise exception '과목·월·목표 횟수를 확인해 주세요.'; end if;
 perform pg_advisory_xact_lock(hashtextextended('monthly-target:'||p_student::text||':'||p_subject||':'||m::text,0));
 select version into current_version from public.student_monthly_lesson_targets where student_id=p_student and subject=p_subject and month=m for update;
 if current_version is distinct from p_expected_version then raise exception '목표가 다른 곳에서 변경되었습니다. 새로고침해 주세요.'; end if;
 insert into public.student_monthly_lesson_targets(student_id,subject,month,target) values(p_student,p_subject,m,p_target)
 on conflict(student_id,subject,month) do update set target=excluded.target,version=student_monthly_lesson_targets.version+1,updated_at=now();
end $$;
revoke all on function public.admin_save_monthly_lesson_target(uuid,text,date,integer,integer) from public,anon;
grant execute on function public.admin_save_monthly_lesson_target(uuid,text,date,integer,integer) to authenticated;

create function public.staff_monthly_lesson_coverage(p_month date)
