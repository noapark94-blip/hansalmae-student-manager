...
type Student={id:string;name:string;school:string|null;grade:string|null;status:Status|"excused"|null;lateMinutes:number|null;absenceReason:string|null;note:string|null;directAdded?:boolean};
...
export type ExamCategory={id:string;name:string;isActive:boolean;sortOrder:number};
...
export function ClassLearningBoard({supabase,classId,date,students,validDay,onDate,onReload}:{supabase:SupabaseClient;classId:string;date:string;students:Student[];validDay:boolean;onDate:(v:string)=>void;onReload:()=>Promise<void>}){
  ...
  return <section className="class-learning-board" ref={rootRef} spellCheck={false}>
    ...
    {!validDay&&!makeupEnabled?<section className="learning-common-record" style={{marginBottom:16}}><div className="learning-common-title"><div><small>정규 수업일 아님</small><b>{date} 보강 수업 기록</b></div><button type="button" className="primary learning-makeup-activate" disabled={makeupLoading} onClick={()=>void activateMakeupDay()}>{makeupLoading?"등록 중…":"+ 보강 수업 기록"}</button></div><p style={{margin:0,color:"#6b6570",fontSize:14,lineHeight:1.6}}>이 날짜는 정규 수업 요일이 아닙니다. 보강 수업으로 등록하면 현재 수강생을 대상으로 출결·수업내용·숙제·시험을 평소와 똑같이 기록할 수 있습니다.</p>{error?<p className="form-error learning-board-error">{error}</p>:null}</section>:<>
    {!validDay&&makeupEnabled?<section className="class-day-notice" style={{marginBottom:16,display:"flex",alignItems:"center",justifyContent:"space-between",gap:12}}><span>보강 수업일로 등록된 날짜입니다. 아래에서 평소 수업과 동일하게 기록할 수 있습니다.</span><button type="button" className="danger-button learning-makeup-deactivate" disabled={makeupLoading} onClick={()=>void deactivateMakeupDay()}>{makeupLoading?"해제 중…":"보강 수업 해제"}</button></section>:null}
    <section className={`learning-compact-common ${commonOpen?"open":"collapsed"}`}>
      <div className="learning-compact-heading"><button type="button" className="learning-common-toggle" aria-expanded={commonOpen} onClick={()=>setCommonOpen(value=>!value)}><span><small>반 공통 기록</small><b>빠른 공통 입력</b></span><strong>{[lessonContent,commonHomework,commonExamTitle||commonExamType,notice].filter(value=>value.trim()).length}/4 입력 <i aria-hidden="true">{commonOpen?"⌃":"⌄"}</i></strong></button><span><button type="button" className="learning-common-action" disabled={templateLoading} onClick={()=>void loadPreviousTemplate()}>{templateLoading?"불러오는 중…":"지난 수업 불러오기"}</button><button type="button" className="learning-common-action" onClick={clearCommon}>일괄입력 비우기</button></span></div>
      {commonOpen?<div className="learning-common-grid">
        <section className={`learning-common-card ${lessonContent.trim()?"filled":""}`}><header><span><small>수업</small><b>공통 수업 내용</b></span><em>{lessonContent.trim()?"입력됨":"미입력"}</em></header><textarea value={lessonContent} onChange={e=>setLessonContent(e.target.value)} placeholder="교재·단원·핵심 수업 내용" rows={4}/><button type="button" className="learning-apply-button" onClick={applyLessonContent}>전체 적용</button></section>
        <section className={`learning-common-card ${commonHomework.trim()?"filled":""}`}><header><span><small>과제</small><b>공통 숙제</b></span><em>{commonHomework.trim()?"입력됨":"미입력"}</em></header><textarea value={commonHomework} onChange={e=>setCommonHomework(e.target.value)} placeholder="교재·페이지·문제 번호·제출일" rows={4}/><button type="button" className="learning-apply-button" onClick={applyHomework}>전체 적용</button></section>
        <section className={`learning-common-card disclosure ${commonExamTitle.trim()||commonExamType.trim()?"filled":""} ${bulkOpen?"open":""}`}><button type="button" className="learning-card-disclosure" onClick={()=>setBulkOpen(value=>!value)}><span><small>평가</small><b>공통 시험</b></span><em>{commonExamTitle||commonExamType||"미입력"}</em><strong>{bulkOpen?"접기":"입력"}</strong></button>{bulkOpen?<div className="learning-card-exam"><select value={commonExamType} onChange={e=>setCommonExamType(e.target.value)}><option value="">종류 선택</option>{categories.map(c=><option value={c.name} key={c.id}>{c.name}</option>)}</select><input value={commonExamTitle} onChange={e=>setCommonExamTitle(e.target.value)} placeholder="시험명·범위"/><span className="learning-common-score"><input value="" readOnly aria-label="원점수는 학생별 입력" placeholder="학생별 입력"/><em>/</em><input value={commonExamMax} onChange={e=>setCommonExamMax(e.target.value)} inputMode="decimal" placeholder="만점"/></span><input value={commonEvaluation} onChange={e=>setCommonEvaluation(e.target.value)} placeholder="평가·피드백"/><button type="button" className="learning-apply-button" onClick={applyExam}>전체 적용</button><small>종류·시험명·만점·평가를 전체 학생에게 적용하고 원점수만 학생별로 입력합니다.</small></div>:<p>시험이 있을 때만 입력하세요.</p>}</section>
        <section className={`learning-common-card disclosure ${notice.trim()?"filled":""} ${noticeOpen?"open":""}`}><button type="button" className="learning-card-disclosure" onClick={()=>setNoticeOpen(value=>!value)}><span><small>안내</small><b>반 전체 공지</b></span><em>{notice||"미입력"}</em><strong>{noticeOpen?"접기":"입력"}</strong></button>{noticeOpen?<textarea value={notice} onChange={e=>setNotice(e.target.value)} placeholder="준비물·일정·반 전체 안내" rows={5}/>:<p>공지가 있을 때만 입력하세요.</p>}</section>
      </div>:null}
    </section>
    ...
    {loading?<p className="settings-empty">불러오는 중이에요…</p>:<div className="learning-board-rows">{rows.map(row=>{const exam=row.exams[0],score=Number(exam.score),max=Number(exam.maxScore);const converted=exam.score!==""&&Number.isFinite(score)&&Number.isFinite(max)&&max>0?Math.round(score/max*1000)/10:null;return <article key={row.id}>
      <div className="learning-person-attendance"><span className="learning-student"><button type="button" className="learning-student-history-button" onClick={()=>setHistoryStudent(row)} title={`${row.name} 학생 누적 수업 기록 보기`}><i>{row.name[0]}</i><b>{row.name}{row.directAdded?<em className="lesson-direct-badge">직접 추가</em>:null}</b><small>{[row.school,row.grade].filter(Boolean).join(" · ")}</small></button></span><div className="learning-attendance">{attendance.map(([status,label])=><button key={status} className={`${status} ${row.status===status?"active":""}`} disabled={saving===row.id} onClick={()=>void saveAttendance(row,status)}>{label}</button>)}{row.status==="late"?<small>{row.lateMinutes}분 지각 · 같은 버튼을 다시 누르면 취소</small>:null}{row.status==="absent"?<small>{row.absenceReason?`${row.absenceReason} · `:""}같은 버튼을 다시 누르면 취소</small>:null}{row.status==="present"?<small>같은 버튼을 다시 누르면 취소</small>:null}</div></div>
...
function FamilyReportReadStatus({supabase,classId,date}:{supabase:SupabaseClient;classId:string;date:string}){
  ...
  return <section className={`family-read-status ${oldUnconfirmed?"needs-attention":""}`}><button type="button" className="family-read-summary" onClick={()=>setOpen(v=>!v)} disabled={loading}><span className="family-read-title"><small>학부모 확인 현황</small><b>{!data?.lessonId?"리포트 생성 전":<>확인 <em>{confirmed}</em> / {data.totalStudents}명</>}</b>{oldUnconfirmed?<em className="family-read-warning">2일 이상 미확인</em>:null}</span><span className="family-read-pills">{data?.lessonId&&unconfirmed?<em className="unconfirmed">미확인 {unconfirmed}명</em>:null}{unlinked?<em className="unlinked">미연결 {unlinked}명</em>:null}<strong>{open?"접기":"학생별 보기 ›"}</strong></span></button>{open&&data?<div className="family-read-details">{data.students.map(student=><article key={student.studentId} className={oldUnconfirmed&&student.status==="unconfirmed"?"stale-unconfirmed":""}><span><b>{student.studentName}</b><small>{[student.school,student.grade].filter(Boolean).join(" · ")||"학생 정보"}</small></span><span className={`family-read-state ${student.status}`}>{student.status==="confirmed"?"학부모 확인":student.status==="unconfirmed"?(oldUnconfirmed?"오래 미확인":"미확인"):"학부모 계정 미연결"}{student.status==="confirmed"&&student.viewedAt?<small>{formatReadTime(student.viewedAt)}</small>:null}</span></article>)}{!data.students.length?<p>이 날짜의 수강 학생이 없습니다.</p>:null}<footer><span>보호자 한 명이라도 확인하면 학부모 확인으로 표시됩니다.</span><button type="button" onClick={()=>void load()}>새로고침</button></footer></div>:null}</section>;
}

export function ExamCategoryModal({supabase,categories,onClose,onChanged}:{supabase:SupabaseClient;categories:ExamCategory[];onClose:()=>void;onChanged:()=>Promise<void>}){
