...
export function calendarValues(v:{reminderRecipientIds?:string[]|null;reminderEnabled?:boolean;scope:string;category:string;startsOn:string;endsOn:string;startsAt:string|null;endsAt:string|null;school:string|null;grade:string|null;title:string;classId:string|null;teacherId:string|null;note:string|null;contactName:string|null;contactPhone:string|null;location:string|null;status:string}) {
  return {reminderRecipientIds:[...new Set(v.reminderRecipientIds??(v.teacherId?[v.teacherId]:[]))].sort(),reminderEnabled:v.reminderEnabled??false,kind:{scope:v.scope,category:v.category},timing:{startsOn:v.startsOn,endsOn:v.endsOn,startsAt:v.startsAt?.slice(0,5)??"",endsAt:v.endsAt?.slice(0,5)??""},
