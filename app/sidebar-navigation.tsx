...
export function SidebarNavigation({ supabase, role: _role, items, activeView, onSelect }: { supabase: SupabaseClient; role: UserRole; items: MenuItem[]; activeView: View; onSelect: (view: View) => void }) {
  ...
  const openItem=(id:View)=>{if(id==="corrections"){window.location.href="/corrections";return;}onSelect(id);};
  ...
  return <>
    <nav aria-label="주요 메뉴" className="folder-navigation">
      ...
      {folders.map((folder) => <section className="nav-folder" key={folder.id}>
        ...
        {!collapsed[folder.id] && <div className="nav-folder-items">{folder.itemIds.map((id) => { const item = itemMap.get(id); return item ? <button type="button" key={id} className={activeView === id ? "active" : ""} onClick={() => openItem(id)}><span className="nav-icon"><HansalmaeIcon name={viewIcon[id]} /></span>{id==="corrections"?"첨삭 관리":item.label}</button> : null; })}</div>}
      </section>)}
    </nav>
