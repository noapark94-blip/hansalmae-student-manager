...
as $$
  select case when public.is_staff() then jsonb_build_object(
    'subjects',coalesce((
      ...
      ) order by case s.main_subject when '국어' then 1 when '영어' then 2 else 3 end,s.parent_id nulls first,s.name)
      ...
      where c.active
        and (
          ...
          or exists(
            ...
            where ct.class_id=c.id and ct.profile_id=auth.uid()
          )
        )
    ),'[]'::jsonb)
  ) else null end
...
revoke all on function public.teacher_class_workspace() from public,anon;
