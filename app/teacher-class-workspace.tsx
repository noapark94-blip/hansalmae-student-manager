  ...
  return (
    <div className="modal-backdrop">
      <form className="modal class-creator" onSubmit={submit}>
        ...
        <div className="modal-body">
          <div className="form-grid"><label>클래스 이름 *<input value={name} onChange={(event)=>setName(event.target.value)} placeholder="예: 고2 영어 서해" /></label><label>과목 *<select value={subjectId} onChange={(event)=>setSubjectId(event.target.value)}>{subjects.map((item)=><option value={item.id} key={item.id}>{item.name}</option>)}</select></label><label>강의실<input value={room} onChange={(event)=>setRoom(event.target.value)} placeholder="예: A 강의실" /></label><label>표시 색상<div className="class-color-options">{["#a92d68","#4c86a8","#c85c7d","#df8658","#c8952a","#5f9074","#7060a7","#61676f"].map((item)=><button type="button" aria-label={`색상 ${item}`} key={item} className={color===item?"active":""} onClick={()=>setColor(item)} style={{background:item}} />)}<input type="color" value={color} onChange={(event)=>setColor(event.target.value)} /></div></label></div>
          ...
          <fieldset className="schedule-builder"><legend>수업 요일·시간 *</legend>{schedules.map((item,index)=><div className="schedule-row" key={index}><div className="weekday-options">{weekdays.map((label,dayIndex)=><button type="button" key={label} className={item.weekdays.includes(dayIndex+1)?"active":""} onClick={()=>updateSchedule(index,{weekdays:item.weekdays.includes(dayIndex+1)?item.weekdays.filter((day)=>day!==dayIndex+1):[...item.weekdays,dayIndex+1]})}>{label}</button>)}</div><label>시작<MilitaryTimeInput value={item.startTime} onChange={(value)=>updateSchedule(index,{startTime:value})}/></label><span>–</span><label>종료<MilitaryTimeInput value={item.endTime} onChange={(value)=>updateSchedule(index,{endTime:value})}/></label>{schedules.length>1&&<button type="button" className="danger-link" onClick={()=>setSchedules((current)=>current.filter((_,idx)=>idx!==index))}>삭제</button>}</div>)}<button type="button" className="secondary-button" onClick={addSchedule}>＋ 다른 시간 추가</button></fieldset>
        </div>
        {error && <p className="form-error">{error}</p>}
...
function ClassEditor({ supabase, profile, subjects, classRoom, onClose, onSaved }: { supabase: SupabaseClient; profile: Profile; subjects: Subject[]; classRoom: ManagedClass; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  return <div className="nested-modal-backdrop"><form className="modal class-creator" onSubmit={submit}><header><div><p className="eyebrow">클래스 수정</p><h2>{classRoom.name}</h2><p>기본 정보·담당 선생님·정규 수업 시간을 함께 수정합니다.</p></div><button type="button" className="modal-close" onClick={onClose}>×</button></header><div className="modal-body"><div className="form-grid"><label>클래스 이름 *<input value={name} onChange={(e)=>setName(e.target.value)}/></label><label>과목 *<select value={subjectId} onChange={(e)=>setSubjectId(e.target.value)}>{subjects.map((item)=><option value={item.id} key={item.id}>{item.name}</option>)}</select></label><label>강의실<input value={room} onChange={(e)=>setRoom(e.target.value)}/></label><label>표시 색상<div className="class-color-options">{["#a92d68","#4c86a8","#c85c7d","#df8658","#c8952a","#5f9074","#7060a7","#61676f"].map((item)=><button type="button" aria-label={`색상 ${item}`} key={item} className={color===item?"active":""} onClick={()=>setColor(item)} style={{background:item}}/>)}<input type="color" value={color} onChange={(e)=>setColor(e.target.value)}/></div></label></div><fieldset className="teacher-picker"><legend>담당 선생님 * <small>공동 담당 가능</small></legend><div>{teachers.map((item)=><label key={item.id} className={teacherIds.includes(item.id)?"active":""}><input type="checkbox" checked={teacherIds.includes(item.id)} onChange={()=>setTeacherIds((current)=>current.includes(item.id)?current.filter((id)=>id!==item.id):[...current,item.id])}/><i>{item.name.slice(0,1)}</i><span>{item.name}</span></label>)}</div></fieldset><fieldset className="schedule-builder"><legend>수업 요일·시간 *</legend>{schedules.map((item,index)=><div className="schedule-row" key={index}><div className="weekday-options">{weekdays.map((label,dayIndex)=><button type="button" key={label} className={item.weekdays.includes(dayIndex+1)?"active":""} onClick={()=>updateSchedule(index,{weekdays:item.weekdays.includes(dayIndex+1)?item.weekdays.filter((day)=>day!==dayIndex+1):[...item.weekdays,dayIndex+1]})}>{label}</button>)}</div><label>시작<MilitaryTimeInput value={item.startTime} onChange={(value)=>updateSchedule(index,{startTime:value})}/></label><span>–</span><label>종료<MilitaryTimeInput value={item.endTime} onChange={(value)=>updateSchedule(index,{endTime:value})}/></label>{schedules.length>1&&<button type="button" className="danger-link" onClick={()=>setSchedules((current)=>current.filter((_,idx)=>idx!==index))}>삭제</button>}</div>)}<button type="button" className="secondary-button" onClick={()=>setSchedules((current)=>[...current,{weekdays:[],startTime:"18:00",endTime:"20:00"}])}>＋ 다른 시간 추가</button></fieldset></div>{error&&<p className="form-error">{error}</p>}<footer><button type="button" className="secondary-button" onClick={onClose}>취소</button><button className="primary" disabled={saving}>{saving?"저장 중…":"수정 저장"}</button></footer></form></div>
}
