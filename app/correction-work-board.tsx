...
export function CorrectionWorkBoard({supabase}:{supabase:SupabaseClient}){
  ...
  const renderRow=(row:Occurrence)=>{
    ...
    const examCategory=report.examRange?.startsWith("[종류]")?report.examRange.slice(4).split("\n")[0]:"";
    const updateExamCategory=(value:string)=>{const old=(report.examRange??"").replace(/^\[종류\].*\n?/,"");updateDraft(row,{examRange:value?`[종류]${value}\n${old}`:old})};
    ...
    return <article key={key} className={`${row.assignment.isDateOverride?"correction-date-override-row ":""}${report.published?"correction-mobile-complete":""}`}>
      ...
      <div className={`correction-mobile-student-details ${detailsOpen?"open":""}`}>
        ...
        <div className="correction-desktop-exam-info"><select value={examCategory} onChange={e=>updateExamCategory(e.target.value)}><option value="">종류 선택</option>{categories.map(category=><option key={category.id} value={category.name}>{category.name}</option>)}</select><input value={report.examTitle??""} onChange={e=>updateDraft(row,{examTitle:e.target.value})} placeholder="시험명·범위"/></div>
        <div className="correction-desktop-exam-score"><span><input inputMode="decimal" value={report.examScore??""} onFocus={e=>e.currentTarget.select()} onChange={e=>updateDraft(row,{examScore:e.target.value===""?null:Number(e.target.value)})} placeholder="원점수"/><em>/</em><input inputMode="decimal" value={report.examMaxScore===undefined?100:(report.examMaxScore??"")} onFocus={e=>e.currentTarget.select()} onChange={e=>updateDraft(row,{examMaxScore:e.target.value===""?null:Number(e.target.value)})} placeholder="만점"/></span><small>{converted===null?"100점 환산":`환산 ${converted}점`}</small></div>
        <div className="correction-desktop-exam-evaluation"><input value={report.evaluation??""} onChange={e=>updateDraft(row,{evaluation:e.target.value})} placeholder="평가·피드백"/></div>
        <div className="learning-exam-list correction-mobile-exam-fields"><div className="learning-exam-card"><div className="learning-exam individual correction-exam"><select value={examCategory} onChange={e=>updateExamCategory(e.target.value)}><option value="">종류 선택</option>{categories.map(category=><option key={category.id} value={category.name}>{category.name}</option>)}</select><input value={report.examTitle??""} onChange={e=>updateDraft(row,{examTitle:e.target.value})} placeholder="시험명·범위"/><span><input inputMode="decimal" value={report.examScore??""} onFocus={e=>e.currentTarget.select()} onChange={e=>updateDraft(row,{examScore:e.target.value===""?null:Number(e.target.value)})} placeholder="원점수"/><em>/</em><input inputMode="decimal" value={report.examMaxScore===undefined?100:(report.examMaxScore??"")} onFocus={e=>e.currentTarget.select()} onChange={e=>updateDraft(row,{examMaxScore:e.target.value===""?null:Number(e.target.value)})} placeholder="만점"/></span><input value={report.evaluation??""} onChange={e=>updateDraft(row,{evaluation:e.target.value})} placeholder="평가·피드백"/></div><small className="exam-percent">{converted===null?"점수를 입력하면 100점 환산점수가 표시됩니다.":`원점수 ${score}/${max} · 환산 ${converted}점`}</small></div></div>
  ...
  return <section className="class-learning-board correction-learning-board" spellCheck={false}>
    ...
    <div className="learning-board-heading correction-learning-heading"><span>학생·출결</span><span>시험 종류·범위</span><span>점수</span><span>평가·피드백</span><span>오늘 한 첨삭과제</span></div>
