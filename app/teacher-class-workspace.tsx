...
import { ClassSettingsSwap } from "./class-settings-swap";
import settingsSwapStyles from "./class-settings-swap.module.css";
...
type ClassScheduleChoice = Schedule & { assigned: boolean };
...
function ClassEditor({ supabase, profile, subjects, classes, classRoom, onClose, onSaved }: { supabase: SupabaseClient; profile: Profile; subjects: Subject[]; classes: ManagedClass[]; classRoom: ManagedClass; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  const [baseline,setBaseline]=useState(()=>classSettingsValues(classRoom));
  const busy=useRef(false);
  const [swapping,setSwapping]=useState(false),[swapBusy,setSwapBusy]=useState(false);
  const openSwap=async()=>{
    if(saving||busy.current)return;
    const mine=classSettingsValues({name,subjectId,room,color,teachers:teacherIds.map(id=>({id})),schedules:schedules.flatMap(item=>item.weekdays.map(weekday=>({weekday,startTime:item.startTime,endTime:item.endTime})))});
    if(Object.keys(settingsChanges(baseline,mine)).length&&!await appConfirm({eyebrow:"입력 중인 변경사항",title:"저장된 시간표로 맞바꿀까요?",copy:"현재 입력 중인 변경사항은 저장되지 않습니다. 시간 맞바꾸기를 완료하면 이 창이 닫힙니다. 돌아오면 입력 내용은 그대로 유지됩니다.",confirmLabel:"맞바꾸기로 이동"}))return;
    setSwapping(true);
  };
  ...
  const submit=async(event:FormEvent)=>{
    ...
    if(!name.trim()||!subjectId||(profile.role!=="admin"&&!teacherIds.length)||schedules.some(item=>!item.weekdays.length||!isMilitaryTime(item.startTime)||!isMilitaryTime(item.endTime))){setError("필수 정보를 모두 입력해 주세요.");return;}
    busy.current=true;setSaving(true);setError("");
    try{
      const mine=classSettingsValues({name,subjectId,room,color,teachers:teacherIds.map(id=>({id})),schedules:schedules.flatMap(item=>item.weekdays.map(weekday=>({weekday,startTime:item.startTime,endTime:item.endTime})))});
      const {data:result,error:failure}=await supabase.rpc("staff_patch_class_settings",{p_id:classRoom.id,p_base:baseline,p_changes:settingsChanges(baseline,mine)});
      if(failure)throw failure;
      if(result.conflicts?.length){
        const latest=result.values as typeof mine;
        const labels:Record<string,string>={name:"클래스 이름",subjectId:"과목",room:"강의실",color:"표시 색상",teacherIds:"담당 선생님",schedules:"수업 요일·시간"};
        const format=(key:string,value:unknown)=>{if(key==="subjectId")return subjects.find(x=>x.id===value)?.name??"과목 없음";if(key==="teacherIds")return (value as string[]).map(id=>teachers.find(x=>x.id===id)?.name??classRoom.teachers.find(x=>x.id===id)?.name??"담당 선생님").join(", ")||"담당 없음";if(key==="schedules")return (value as typeof mine.schedules).map(x=>`${weekdays[x.weekday-1]} ${x.startTime}–${x.endTime}`).join(" / ");return String(value||"미입력");};
        const choices=await compareEdits(result.conflicts.map((key:keyof typeof mine)=>({id:key,student:classRoom.name,field:labels[key],mine:format(key,mine[key]),latest:format(key,latest[key])})));
        if(choices){const merged=mergeSettingsChoices(baseline,mine,latest,choices);setBaseline(latest);setName(merged.name);setSubjectId(merged.subjectId);setRoom(merged.room);setColor(merged.color);setTeacherIds(merged.teacherIds);setSchedules(merged.schedules.map(x=>({weekdays:[x.weekday],startTime:x.startTime,endTime:x.endTime})));setError("선택한 내용을 적용했습니다. 확인 후 다시 저장해 주세요.");}
        return;
      }
      await onSaved();
    }catch(e){setError((e as {message?:string}).message??"클래스 저장에 실패했습니다.");}finally{busy.current=false;setSaving(false);}
  };
  if(swapping)return <div className="nested-modal-backdrop"><section className={`modal class-creator ${settingsSwapStyles.dialog}`} role="dialog" aria-modal="true" aria-labelledby="class-settings-swap-title"><header><div><p className="eyebrow">클래스 수정 · 시간 맞바꾸기</p><h2 id="class-settings-swap-title">{classRoom.name}</h2><p>선택한 요일의 두 수업을 한 번에 변경합니다.</p></div><button type="button" className="modal-close" aria-label="클래스 수정으로 돌아가기" disabled={swapBusy} onClick={()=>setSwapping(false)}>×</button></header><div className="modal-body"><ClassSettingsSwap supabase={supabase} classId={classRoom.id} onCancel={()=>setSwapping(false)} onSaved={onSaved} onBusyChange={setSwapBusy}/></div></section></div>;
  return <div className="nested-modal-backdrop"><form className="modal class-creator" onSubmit={submit} inert={saving}><header><div><p className="eyebrow">클래스 수정</p><h2>{classRoom.name}</h2><p>기본 정보·담당 선생님·정규 수업 시간을 함께 수정합니다.</p></div><button type="button" className="modal-close" onClick={onClose}>×</button></header><div className="modal-body"><div className="form-grid"><label>클래스 이름 *<input value={name} onChange={(e)=>setName(e.target.value)}/></label><label>과목 *<select value={subjectId} onChange={(e)=>setSubjectId(e.target.value)}>{subjects.map((item)=><option value={item.id} key={item.id}>{item.name}</option>)}</select></label><label>강의실<input value={room} onChange={(e)=>setRoom(e.target.value)}/></label><label>표시 색상<ClassColorPicker value={color} onChange={setColor} classes={classes} excludeClassId={classRoom.id}/></label></div><fieldset className="teacher-picker"><legend>담당 선생님{profile.role!=="admin"?" *":""} <small><span className="desktop-only-hint">{profile.role==="admin"?"나중에 배정 가능":"공동 담당 가능"}</span><span className="mobile-only-hint">{profile.role==="admin"?"선택하지 않으면 나중에 배정":"여러 명 선택 가능"}</span></small></legend><div>{teachers.map((item)=><label key={item.id} className={teacherIds.includes(item.id)?"active":""}><input type="checkbox" checked={teacherIds.includes(item.id)} onChange={()=>setTeacherIds((current)=>current.includes(item.id)?current.filter((id)=>id!==item.id):[...current,item.id])}/><i>{item.name.slice(0,1)}</i><span>{item.name}</span></label>)}</div></fieldset><fieldset className="schedule-builder"><legend>수업 요일·시간 *</legend><button type="button" className={settingsSwapStyles.entry} onClick={()=>void openSwap()}><span><b>다른 수업과 시간 맞바꾸기</b><small>요일을 골라 두 수업의 시간을 한 번에 변경해요</small></span><span aria-hidden="true">⇄</span></button>{schedules.map((item,index)=><div className="schedule-row" key={index}><b className="schedule-field-title weekday-title">요일</b><b className="schedule-field-title start-title">시작</b><b className="schedule-field-title end-title">종료</b><div className="weekday-options">{weekdays.map((label,dayIndex)=><button type="button" key={label} className={item.weekdays.includes(dayIndex+1)?"active":""} onClick={()=>updateSchedule(index,{weekdays:item.weekdays.includes(dayIndex+1)?item.weekdays.filter((day)=>day!==dayIndex+1):[...item.weekdays,dayIndex+1]})}>{label}</button>)}</div><div className="schedule-time-input start-time-input"><MilitaryTimeInput value={item.startTime} onChange={(value)=>updateSchedule(index,{startTime:value})}/></div><span aria-hidden="true">→</span><div className="schedule-time-input end-time-input"><MilitaryTimeInput value={item.endTime} onChange={(value)=>updateSchedule(index,{endTime:value})}/></div>{schedules.length>1&&<button type="button" className="danger-link" onClick={()=>setSchedules((current)=>current.filter((_,idx)=>idx!==index))}>삭제</button>}</div>)}<small className="schedule-time-hint"><span className="desktop-only-hint">시간은 24시간제 4자리로 입력해 주세요. 예: 1800</span><span className="mobile-only-hint">숫자 4자리 입력 시 자동으로 표시됩니다. 예: 1800 → 18:00</span></small><button type="button" className="add-schedule-row" onClick={()=>setSchedules((current)=>[...current,{weekdays:[],startTime:"18:00",endTime:"20:00"}])}>＋ 다른 시간 추가</button></fieldset></div>{error&&<p className="form-error">{error}</p>}<footer><button type="button" className="secondary-button" onClick={onClose}>취소</button><button className="primary" disabled={saving}>{saving?"저장 중…":"수정 저장"}</button></footer></form></div>
}
...
function ClassRosterEditor({ supabase, classRoom, onClose, onSaved }: { supabase: SupabaseClient; classRoom: ClassRoom; onClose: () => void; onSaved: () => Promise<void> }) {
  const [students,setStudents]=useState<RosterStudent[]>([]),[loading,setLoading]=useState(true),[saving,setSaving]=useState(""),[query,setQuery]=useState(""),[error,setError]=useState(""),[scheduleStudent,setScheduleStudent]=useState<RosterStudent|null>(null);
  const load=useCallback(async()=>{setLoading(true);const{data:rows,error:loadError}=await supabase.rpc("staff_students_with_enrollments");if(loadError)setError(loadError.message);else setStudents((rows as RosterStudent[])??[]);setLoading(false)},[supabase]);
  useEffect(()=>{void load()},[load]);
  ...
  const toggle=async(student:RosterStudent,currentlyEnrolled:boolean)=>{if(!currentlyEnrolled&&classRoom.schedules.length>1){setScheduleStudent(student);return}setSaving(student.id);setError("");const{error:saveError}=await supabase.rpc("staff_set_class_enrollment",{p_class_id:classRoom.id,p_student_id:student.id,p_active:!currentlyEnrolled});if(saveError)setError(saveError.message);else await Promise.all([load(),onSaved()]);setSaving("")};
  return <div className="nested-modal-backdrop"><div className="modal roster-editor"><header><div><p className="eyebrow">수강학생 관리</p><h2>{classRoom.name}</h2><p>여러 요일 수업은 실제 참석 시간을 먼저 선택한 뒤 학생을 추가합니다.</p></div><button className="modal-close" aria-label="닫기" onClick={onClose}>×</button></header><div className="modal-body"><input className="roster-search" value={query} onChange={(e)=>setQuery(e.target.value)} placeholder="학생 이름 또는 학교 검색"/>{loading?<p className="empty-state">학생을 불러오는 중이에요…</p>:<div className="roster-student-list">{sorted.map((item)=>{const enrolled=enrolledIds.has(item.id);return <article key={item.id} className={enrolled?"enrolled":""}><span><b>{item.name}</b><small>{[item.school,item.grade].filter(Boolean).join(" · ")||"학교·학년 미입력"}</small></span><div>{enrolled&&classRoom.schedules.length>1?<button type="button" className="schedule-button" onClick={()=>setScheduleStudent(item)}>요일 설정</button>:null}<button type="button" className={enrolled?"remove-button":"add-button"} disabled={saving===item.id} onClick={()=>void toggle(item,enrolled)}>{saving===item.id?"처리 중…":enrolled?"제외":"추가"}</button></div></article>})}{!sorted.length?<p className="empty-state">검색 결과가 없습니다.</p>:null}</div>}{error&&<p className="form-error">{error}</p>}</div><footer><button className="primary" onClick={onClose}>완료</button></footer>{scheduleStudent?<ClassStudentScheduleEditor supabase={supabase} classRoom={classRoom} student={scheduleStudent} enrolled={enrolledIds.has(scheduleStudent.id)} onClose={()=>setScheduleStudent(null)} onSaved={async()=>{setScheduleStudent(null);await Promise.all([load(),onSaved()])}}/>:null}</div></div>
}

function ClassStudentScheduleEditor({supabase,classRoom,student,enrolled,onClose,onSaved}:{supabase:SupabaseClient;classRoom:ClassRoom;student:RosterStudent;enrolled:boolean;onClose:()=>void;onSaved:()=>Promise<void>}){
  const [baseline,setBaseline]=useState<unknown>(null);
  const [choices,setChoices]=useState<ClassScheduleChoice[]>([]),[selectedIds,setSelectedIds]=useState<string[]>([]),[loading,setLoading]=useState(true),[saving,setSaving]=useState(false),[error,setError]=useState("");
  useEffect(()=>{let active=true;void(async()=>{const{data,error:loadError}=await supabase.rpc("staff_class_weekday_snapshot",{p_class_id:classRoom.id,p_student_id:student.id});if(!active)return;if(loadError)setError(loadError.message);else{setBaseline(data.base);const rows=((data.choices as Array<{scheduleId:string;weekday:number;startTime:string;endTime:string;assigned:boolean}>)??[]).map((item)=>({id:item.scheduleId,weekday:item.weekday,startTime:item.startTime,endTime:item.endTime,assigned:item.assigned}));setChoices(rows);setSelectedIds(rows.filter((item)=>item.assigned).map((item)=>item.id))}setLoading(false)})();return()=>{active=false}},[classRoom.id,student.id,supabase]);
  const toggleChoice=(scheduleId:string)=>setSelectedIds((current)=>current.includes(scheduleId)?current.filter((id)=>id!==scheduleId):[...current,scheduleId]);
  const save=async()=>{if(saving||loading||!baseline)return;if(!selectedIds.length){setError("최소 한 개의 수업 요일·시간을 선택해 주세요.");return}setSaving(true);setError("");const{error:saveError}=await supabase.rpc("staff_guard_class_weekdays",{p_class_id:classRoom.id,p_student_id:student.id,p_schedule_ids:selectedIds,p_base:baseline});if(saveError){setError(saveError.message);setSaving(false);return}await onSaved()};
  return <div className="nested-modal-backdrop" onMouseDown={(event)=>{if(event.target===event.currentTarget&&!saving)onClose()}}><section className="modal class-student-schedule-modal" role="dialog" aria-modal="true" aria-labelledby="class-student-schedule-title"><header><div><p className="eyebrow">요일별 수강 배정</p><h2 id="class-student-schedule-title">{student.name}</h2><p>{classRoom.name}에서 실제로 참석하는 시간만 선택합니다.</p></div><button type="button" className="modal-close" aria-label="닫기" disabled={saving} onClick={onClose}>×</button></header><div className="modal-body"><div className="schedule-scope-note"><strong>다른 과목·반은 그대로 유지돼요</strong><span>선택한 시간만 충돌을 확인해 {enrolled?"시간표를 변경":"학생을 추가"}합니다.</span></div>{loading?<p className="empty-state">수업 시간을 불러오는 중이에요…</p>:choices.length?<div className="student-schedule-choice-list">{choices.map((choice)=>{const selected=selectedIds.includes(choice.id);return <button type="button" key={choice.id} className={selected?"selected":""} aria-pressed={selected} disabled={saving} onClick={()=>toggleChoice(choice.id)}><i aria-hidden="true">{selected?"✓":""}</i><span><b>{weekdays[choice.weekday-1]}요일</b><small>{choice.startTime.slice(0,5)}–{choice.endTime.slice(0,5)}</small></span></button>})}</div>:<p className="empty-state">먼저 클래스에 수업 요일·시간을 등록해 주세요.</p>}{error?<p className="form-error">{error}</p>:null}</div><footer><button type="button" className="secondary-button" disabled={saving} onClick={onClose}>취소</button><button type="button" className="primary" disabled={loading||saving||!choices.length} onClick={()=>void save()}>{saving?"저장 중…":enrolled?`선택한 ${selectedIds.length}개 저장`:`선택한 ${selectedIds.length}개로 추가`}</button></footer></section></div>
}
