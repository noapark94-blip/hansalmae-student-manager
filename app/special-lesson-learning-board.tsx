...
export function SpecialLessonLearningBoard({ supabase, profile, sessionId, lessonKind, onClose, onAttendanceChange, embedded = false }: { supabase: SupabaseClient; profile: Profile; sessionId: string; lessonKind: "makeup"|"additional"; onClose: () => void; onAttendanceChange?: (change?:{studentId:string;status:Status|null}) => void | Promise<void>; embedded?: boolean }) {
  ...
  return <section inert={editingSchedule||reviewing} className={`${embedded ? "class-learning-board special-board-embedded" : "student-modal"} special-board-modal special-record-viewport`} spellCheck={false}>
    ...
    {historyStudent?<div className="modal-backdrop nested" onMouseDown={event=>{if(event.target===event.currentTarget)setHistoryStudent(null)}}><section className="student-modal student-learning-history-modal" role="dialog" aria-modal="true"><header><div><p className="eyebrow">교직원 전용 · 누적 수업 기록</p><h2>{historyStudent.name}<small className="history-type-label">{(historyStudent.lessonKind??lessonKind)==="makeup"?"보강수업 기록":"추가수업 기록"}</small></h2><span>{[historyStudent.school,historyStudent.grade].filter(Boolean).join(" · ")||"학생 기록"}</span></div><button type="button" aria-label="닫기" onClick={()=>setHistoryStudent(null)}>×</button></header><StudentLearningHistory supabase={supabase} studentId={historyStudent.id} initialSource={historyStudent.lessonKind??lessonKind}/></section></div>:null}
  </section>;
}
