...
import {CorrectionClosureBanner} from "./correction-closure-banner";
...
type Board={closureDays?:{date:string;reason:string}[];assignments:Assignment[];exceptions:Exception[]};
...
export function CorrectionWorkBoard({supabase,initialDate,focusStudentId,focusAssignmentId,focusTime}:{supabase:SupabaseClient;initialDate?:string;focusStudentId?:string;focusAssignmentId?:string;focusTime?:string}){
  ...
  useStaffLiveUpdates(supabase,`correction-closures-${date}`,"key=eq.academy-closures",async()=>{await load();},failure=>setError((failure as {message?:string}).message??"휴강 설정을 갱신하지 못했습니다."));
  ...
  const renderRow=(row:Occurrence)=>{
    ...
    return <article key={key} className={`${row.assignment.isDateOverride?"correction-date-override-row ":""}${report.published?"correction-mobile-complete":""}`}>
      ...
      <div className={`correction-mobile-student-details ${detailsOpen?"open":""}`}>
        ...
        <div className="correction-mobile-detail-label"><span>오늘의 첨삭 과제</span><small>내용·수행 상태·검사 피드백</small></div>
        <div className="correction-task-cell"><textarea value={report.correctionContent??""} onChange={e=>updateDraft(row,{correctionContent:e.target.value})} placeholder="오늘의 첨삭 과제 내용을 입력하세요." rows={3}/><div className="correction-task-check"><select aria-label="첨삭 과제 수행 상태" value={report.correctionTaskStatus??""} onChange={e=>updateDraft(row,{correctionTaskStatus:(e.target.value||null) as TaskStatus})}><option value="">수행 상태</option><option value="completed">완료</option><option value="partial">부분 완료</option><option value="incomplete">미완료</option></select><input value={report.correctionTaskFeedback??""} onChange={e=>updateDraft(row,{correctionTaskFeedback:e.target.value})} placeholder="검사 피드백 (선택)"/></div></div>
      </div>
    </article>;
  };
  ...
  return <section className="class-learning-board correction-learning-board" spellCheck={false}>
    ...
    {data?.closureDays?.find(d=>d.date===date)&&<CorrectionClosureBanner key={date} supabase={supabase} date={date} reason={data.closureDays.find(d=>d.date===date)!.reason} students={buildOccurrences(data,date,true).filter(r=>r.assignment.closedDates?.includes(date)).map(r=>({...r.assignment,startTime:r.startTime}))} onApplied={load}/>}
...
function buildOccurrences(data:Board|null,date:string,includeClosed=false):Occurrence[]{if(!data)return[];const weekday=isoWeekday(date);const rows:Occurrence[]=[];for(const a of data.assignments??[]){if(a.weekday===weekday&&isAssignmentValid(a,date)){const x=(data.exceptions??[]).find(e=>e.assignmentId===a.id&&e.originalDate===date&&(e.kind==="move"||e.kind==="cancel"));if(!x)rows.push({assignment:a,date,startTime:a.startTime,endTime:a.endTime,kind:"fixed"})}}for(const x of data.exceptions??[]){if((x.kind==="move"||x.kind==="extra")&&x.targetDate===date&&x.targetStartTime&&x.targetEndTime){const a=(data.assignments??[]).find(v=>v.id===x.assignmentId);if(a)rows.push({assignment:a,date,startTime:x.targetStartTime,endTime:x.targetEndTime,kind:x.kind,exception:x})}}return rows.filter(r=>includeClosed||!r.assignment.closedDates?.includes(date)).sort((a,b)=>a.startTime.localeCompare(b.startTime)||a.assignment.subject.localeCompare(b.assignment.subject,"ko")||a.assignment.studentName.localeCompare(b.assignment.studentName,"ko"))}
